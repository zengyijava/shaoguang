...
/*	private String getMysqlSql(String conditionSql, String tableSql)
	...
	//实时记录的短信内容包括长短信 ，短短信
	public List<SystemMtTaskVo> resendMtMessage(Long ptmsgid,int ptn) throws Exception {
		// TODO Auto-generated method stub
		Connection conn=null;
		PreparedStatement ps=null;
		ResultSet rs=null;
		long ssid=17179869184l;
		StringBuffer stb=new StringBuffer("select message,svrtype,pknumber from (SELECT message,svrtype,pknumber FROM mt_task where  ptmsgid in(");
		String ptmsgidstb = "";
		//拼接SQL
		for(int i=1;i<=ptn;i++){
			ptmsgidstb+=Long.toString(ptmsgid+(i-1)*ssid)+",";
		}

		ptmsgidstb=ptmsgidstb.substring(0,ptmsgidstb.lastIndexOf(","))+")";
		stb.append(ptmsgidstb);
		// 拼接 备份表 实时数据
		stb.append(" union all ");
		stb.append(" SELECT message,svrtype,pknumber FROM GW_MT_TASK_BAK where  ptmsgid in(").append(ptmsgidstb);
		stb.append(" ) a  order by a.pknumber  ");
		List<SystemMtTaskVo> list =new ArrayList<SystemMtTaskVo>();
		try {
			conn=connectionManager.getDBConnection(StaticValue.EMP_POOLNAME);
			ps=conn.prepareStatement(stb.toString());
			rs=ps.executeQuery();
			while(rs.next()){
				SystemMtTaskVo sm=new SystemMtTaskVo();
				sm.setMessage(rs.getString("message"));
				sm.setSvrtype(rs.getString("svrtype"));
				list.add(sm);
			}
		} catch (Exception e) {
			throw e;
