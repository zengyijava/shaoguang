 ...
 * @description
 */
public class SysMtRecordRealDAO extends SuperDAO
{
	/**
	 * 下行实时记录查询
...
//			String fieldSql = "select ID,USERID,SPGATE,CPNO,PHONE,TASKID,SENDSTATUS,ERRORCODE,SENDTIME,MESSAGE,UNICOM,RECVTIME,SVRTYPE,PKNUMBER,PKTOTAL,MSGFMT,P1,P2,usermsgid ";
			...
			//mysql实时表mt_task有两条多余的下行记录，使用 PTMSGID < 0 OR  PTMSGID > 30000000000000条件排除
			//加一个字段custid的值
			//加一个字段custid的值,mt_task表里没有这个字段，赋值为NULL
			//判断conditionSql是否包含where关键字
//			String hasWhere = conditionSql.contains("where") ? "and" : "where";
//			String mysqlTableStr = StaticValue.DBTYPE == StaticValue.MYSQL_DBTYPE ? hasWhere + " PTMSGID < 0 OR  PTMSGID > 30000000000000 " : "";
//			String tableName = "(" +
//					fieldSql + ",custid " + " from " + "gw_mt_task_bak mtt " + conditionSql + StaticValue.WITHNOLOCK + " union all " + fieldSql + ",NULL " + " from " + "mt_task mtt " + conditionSql +
//					StaticValue.WITHNOLOCK + mysqlTableStr + ")";
            // + mysqlTableStr;
            String tableName = " gw_mt_task_bak mttask " + conditionSql + StaticValue.getWITHNOLOCK();

			//获得语句
			String sql = getSql(" from " + tableName);
...
//                tableName = tableName.replaceAll(fieldSql + ",custid", " select count(*) as cnt ");
//                tableName = tableName.replaceAll(fieldSql + ",NULL", " select count(*) as cnt ");
//                countSql = "select sum(cnt) totalcount from " + tableName + " mttask";
//            }

			//读取配置文件里是否启用备用服务器
			String backupServer = SystemGlobals.getValue("montnets.emp.use_backup_server");
			// 启用备用服务器
			if ("1".equals(backupServer)) {
				try {
                    // 用备用服务器查询获取数据
                    returnList = new DataAccessDriver().getGenericDAO().findPageDynaBeanBySQL(sql, countSql, pageInfo, StaticValue.EMP_BACKUP, null);
                    return returnList;
				} catch (Exception e) {
...
//						.append("(")
//						.append(fieldSql).append(" from ").append("gw_mt_task_bak").append(StaticValue.WITHNOLOCK)
//						.append(" union all ")
//						.append(fieldSql).append(" from ").append("mt_task").append(StaticValue.WITHNOLOCK).append(" WHERE PTMSGID < 0 OR  PTMSGID > 30000000000000 ")
//						.append(")").toString();
//			}else
//			{
...
//				.append(fieldSql).append(" from ").append("gw_mt_task_bak").append(StaticValue.WITHNOLOCK)
//				.append(" union all ")
//				.append(fieldSql).append(" from ").append("mt_task").append(StaticValue.WITHNOLOCK)
//				.append(")").toString();
//			}
			//查询表名 
