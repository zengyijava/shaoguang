...
public class LineReportFormImpl implements ReportForm {
...
//			Font rfont = new Font(fmt, bold, size);
			...
			for (int i = 0; i < cmax; i++) {
				ds.addValue((double)i+1, i+"类", "第一季度"); 
				ds.addValue((double)i+1, i+"类", "第二季度"); 
				ds.addValue((double)i+1, i+"类", "第三季度"); 
				ds.addValue((double)i+1, i+"类", "第四季度"); 
            }

			// 创建柱状图
			JFreeChart chart = ChartFactory.createLineChart(cont, "季度", jsonObject.get("dataunit").getAsString(), ds, PlotOrientation.VERTICAL, true, true, true);
			...
			plot.getRangeAxis().setRange(0, (double)cmax+1);
			// 存储成图片
