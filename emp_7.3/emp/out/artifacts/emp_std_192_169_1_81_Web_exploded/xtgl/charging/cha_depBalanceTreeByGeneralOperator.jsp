...
<HTML>
	<HEAD>
		...
		<link rel="stylesheet" type="text/css" href="<%=iPath%>/css/cha_depBalanceTreeByGeneralOperator.css?V=<%=StaticValue.getJspImpVersion() %>"/>
		<link rel="stylesheet" type="text/css" href="<%=skin %>/cha_depBalanceTreeByGeneralOperator.css?V=<%=StaticValue.getJspImpVersion() %>"/>
		...
		<SCRIPT LANGUAGE="JavaScript">
			...
			setting = {
				async : true,
				asyncUrl : "<%=path%>/cha_balanceMgr.htm?method=getOperatorBalancePriDepJson&lguserid="+$("#lguserid").val(), //获取节点数据的URL地址
				isSimpleData : true,
				rootPID : 0,
				treeNodeKey : "id",
				...
				callback : {
					beforeAsync : function(treeId, treeNode) {
					  //  if(treeNode.dlevel < <%=level %>){
					    	zTree.setting.asyncUrl = "<%=path%>/cha_balanceMgr.htm?method=getOperatorBalancePriDepJson&depId="+treeNode.id+
			...
			function getDep(event,treeId,treeNode)
			{
				...
				//新增：将充值回收权限类型设置为非默认的
				$("#operatorBalancePri").val('notDefaultBalancePri');
				getMoney();
				$("#txtPage").val("1");
				submitForm();
			}
	...
	<BODY id="cha_depBalanceTreeByGeneralOperator">
