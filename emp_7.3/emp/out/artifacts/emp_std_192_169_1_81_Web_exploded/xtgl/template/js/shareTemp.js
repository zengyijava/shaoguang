...
function moveIn() {
	...
	{
		alert(getJsLocaleMessage("xtgl","xtgl_cswh_dxmbgl_3"));
		return;
	}
...
function addUser() {
	...
	$("#left option:selected").each(function() {
		...
		if(lfuserid==lguserid||lfuserid==userid){
			alert(getJsLocaleMessage("xtgl","xtgl_cswh_dxmbgl_4"));
			return;
		}
		// 这里判断类型 1 是机构 2是操作员
			...
			if (isflag == "1") {
				$("#right").append("<option value=\'" + lfuserid+ "\' isdeporuser='2'>["+getJsLocaleMessage("xtgl","xtgl_spgl_shlcgl_24")+"]" + name+ "</option>");
			}
		});
	if (tsmsgFlag == "2") {
...
function addDep(depid,depname) {
	...
	if (isflag == "1") {
		$("#right").append("<option value=\'" + depid + "\' isdeporuser='1'>["+getJsLocaleMessage("xtgl","xtgl_spgl_shlcgl_25")+"]"+ depname + "</option>");
	}
/**	if (tsmsgFlag == "2") {
		alert("选择记录重复，将自动过滤！");
	}*/
...
function searchbyname(){
	...
	$.post(pathUrl+"/tem_smsTemplate.htm", {method:"getSysuserByDepId",searchname:searchname,lgcorpcode:lgcorpcode}, function(returnmsg){
