...
<html>
  ...
  <body style="margin:0;padding:0;">
	...
	setting = {
			...
			asyncUrl: "<%=request.getContextPath()%>/smm_sameMms.htm?method=getClientSecondDepJson&lguserid=<%=lguserid%>",  //获取节点数据的URL地址
			...
			callback:{
				...
				beforeAsync: function(treeId, treeNode) {
			    	zTree.setting.asyncUrl="<%=request.getContextPath()%>/smm_sameMms.htm?method=getClientSecondDepJson&depId="+treeNode.id+"&lguserid=<%=lguserid%>";
				}
			}
		};
	...
	function zTreeOnClick(event, treeId, treeNode) {
		...
		$.post("<%=path %>/smm_sameMms.htm", {method:"getClientByDepId", depId:depId,pageIndex:pageIndex}, function(result){
