   $(document).ready(function(){
		 ...
		 function moveLeft() {
				var path = $("#ipath").val();
				var types = $("#chooseType").attr("value");//判断类型
				var showtype="员工";
				if(types == "3"){
					showtype="客户";
				}
					...
					$("#left option:selected").each(function() {
						...
						if (!isExist) {
							if($(this).val().indexOf("m_")>-1)
							{
								...
								$("#getChooseMan").append("<li dataval='"+$(this).val()+"' et='1'  mobile='"+$(this).attr('mobile')+"'>"+" [自建] "+name+"</li>");
								$("#rightSelectTemp").append("<option value=\'"+$(this).val()+"\' mobile=\'"+$(this).attr('mobile')+"\'>"+" [自建] "+name+"</option>");
							}else
							{
								manCount=manCount+1;
								$("#right").append("<option value=\'"+$(this).val()+"\'  et=\'1\' mobile=\'"+$(this).attr('mobile')+"\'>"+" ["+showtype+"] "+name+"</option>");
								$("#getChooseMan").append("<li dataval='"+$(this).val()+"'  et='1' mobile='"+$(this).attr('mobile')+"'>"+" ["+showtype+"] "+name+"</li>");
								$("#rightSelectTemp").append("<option value=\'"+$(this).val()+"\' mobile=\'"+$(this).attr('mobile')+"\'>"+" ["+showtype+"] "+name+"</option>");
							}
						} else {
							if(alertNum==1){
								alert("选择记录重复，将自动过滤！");
