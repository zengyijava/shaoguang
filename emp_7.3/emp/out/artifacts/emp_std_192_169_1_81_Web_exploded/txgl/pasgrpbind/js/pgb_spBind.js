...
setting3 = {			
		async : true,				
		asyncUrl : "pgb_spBind.htm?method=createDeptTree", 			
		checkable : true,
	    checkStyle : "checkbox",
	    checkType : { "Y": "", "N": "" },	
...
function zTreeOnClick3(event, treeId, treeNode) {
	...
	if (treeNode) {
		...
		for(var i=0; i<zTreeNodes3.length; i++){					
			...
			params[i]="id:"+zTreeNodes3[i].id; //拼接机构编号
							
		}				
		$("#depNam").attr("value", pops); //设置机构属性
		$("#deptString").attr("value", params); //设置机构代码	
