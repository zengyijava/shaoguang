...
//	+ "(/[0-9a-z_!~*'().;?:@&=+$,%#-]+)+/?)$";
	...
	var re=new RegExp(strRegex); 
			if(document.forms["form1"].userid.value.length==0 || document.forms["form1"].userid.value.length!=6){
				//window.alert("代理账号不能为空且长度只能为6位字母或数字！");
				window.alert(getJsLocaleMessage("txgl","txgl_wgqdpz_dcspzh_text_31"));
			...
			/*}else if(document.forms["form1"].lxrph.value!=""&&!PhoneCheck(document.forms["form1"].lxrph.value)){				
				...
				$.post("pag_proxyMage.htm",{userid:userid,method:"checkName"},function(result){
					if(result=="false"){
						//alert("该账户已存在，请重新输入！");
