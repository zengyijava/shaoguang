...
function toLogin(obj, account, pass, code, inyzm, phoneword, showMsgStr) {
    $.post(getContextPath + "/login", {
        ...
        if (result == "yzfalse") {
		    ...
		    if(times <6 ){
		    	alert("密码已累计输错"+times+"次，超过6次账户将被锁定，建议自助重置密码！");
		    }else{
		    	alert("用户账户已被锁定，请联系管理员重置密码！");
		    }
		} 
...
function AddFavorite(title, url) {
    try {
        ...
        try {
            ...
            showMsg("抱歉，您所使用的浏览器无法完成此操作。<br /><br />加入收藏失败，请使用Ctrl+D进行添加",msgConfig);
        }
    }
}
...
function showLoading(flag){
	if(flag){
		$('#login_btn').html('<span><img src=' + iPath + '/img/loading.gif>登录中</span>').addClass('login_btn_active');
	}else{
