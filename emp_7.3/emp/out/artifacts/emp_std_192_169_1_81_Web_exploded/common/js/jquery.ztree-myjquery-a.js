...
(function($) {
	...
	$.fn.zTree = function(zTreeSetting, zTreeNodes) {
		...
		if (setting.root[setting.nodesCol] && setting.root[setting.nodesCol].length > 0) {
			...
			asyncGetNode(setting,zTreeNodes);
		}
	...
	function asyncGetNode(setting, treeNode) {
		var i, l;
		var nodeTmp = treeNode;
		if(nodeTmp&&nodeTmp.length==0){nodeTmp = undefined;treeNode.isParent = true;}
		...
		var tmpParam = "";
		for (i = 0, l = setting.asyncParam.length; nodeTmp && i < l; i++) {
			tmpParam += (tmpParam.length > 0 ? "&": "") + setting.asyncParam[i] + "=" + nodeTmp[setting.asyncParam[i]];
		}
		...
		$.ajax({
			type: "POST",
			url: tools.apply(setting.asyncUrl, [nodeTmp], setting.asyncUrl),
			data: tmpParam,
			dataType: "text",
			success: function(msg) {
				...
				setNodeLineIcos(setting, nodeTmp);
				if (newNodes && newNodes != "") {
					newNodes = tools.apply(setting.asyncDataFilter, [setting.treeObjId, nodeTmp, newNodes], newNodes);
					addTreeNodes(setting, nodeTmp, newNodes, false);
				}
				setting.treeObj.trigger(ZTREE_ASYNC_SUCCESS, [setting.treeObjId, nodeTmp, msg]);
			...
			error: function(XMLHttpRequest, textStatus, errorThrown) {
				...
				setNodeLineIcos(setting, nodeTmp);
				if (treeNode) treeNode.isAjaxing = null;
				setting.treeObj.trigger(ZTREE_ASYNC_ERROR, [setting.treeObjId, nodeTmp, XMLHttpRequest, textStatus, errorThrown]);
			}
		});
	}
	...
	function transformTozTreeFormat(setting, simpleTreeNodes) {
		...
		if (tools.isArray(simpleTreeNodes)) {
			var r = [];
			var tmpMap = [];
			for (i=0, l=simpleTreeNodes.length; i<l&&simpleTreeNodes[i]; i++) {
