...
String menuCode = titleMap.get("realTimeAlarm");
...
<html>
  ...
  <body>
    <div id="container" class="container">
		...
		<%-- 表示请求的名称 --%>
		<form name="pageForm" action="mon_realTimeAlarm.htm" method="post" id="pageForm">
		<input type="hidden" id="pathUrl" name="pathUrl" value="<%=path %>">
			...
			<div class="buttons">
				...
				<%if(btnMap.get(menuCode+"-2")!=null) { %>
					<a href="javascript:openDealMons()" id="delcli" title="<emp:message key='ptjk_jkxq_ssgj_1' defVal='处理选中告警信息' fileName='ptjk'/>" style=""><emp:message key="ptjk_jkxq_ssgj_plcl" defVal="批量处理" fileName="ptjk"/></a>
				<%} %>
			</div>
			<div id="condition">
				<table>
					<tbody>
						<tr>
							...
							<td>
								<emp:message key="ptjk_jkxq_ssgj_clzt_mh" defVal="处理状态：" fileName="ptjk"/>
							</td>
							<td >
								<select id="dealflag" name="dealflag" isInput="false">
									<option value=""><emp:message key="ptjk_common_qb" defVal="全部" fileName="ptjk"/></option>
									<option value="1" <%="1".equals(dealflag)?"selected":"" %>><emp:message key="ptjk_jkxq_ssgj_xsj" defVal="新事件" fileName="ptjk"/></option>
									<option value="2" <%="2".equals(dealflag)?"selected":"" %>><emp:message key="ptjk_jkxq_ssgj_clz" defVal="处理中" fileName="ptjk"/></option>
								</select>
							</td>
							<td class="tdSer">
			...
			<table id="content">
				<thead>
					<tr>
						<th>
							<input type="checkbox" name="check" value="" onclick="checkAlls(this,'checklist')"/>
						</th>
						<th>
...
<%--					<th>
				...
				<tr>
					<td>
						<input type="checkbox" name="checklist" value="<%=mon.get("id") %>" />
					</td>
					<td >
						<%
					...
					<td>
						<%=mon.get("montimer")!=null?mon.get("montimer").toString():"-" %>
					</td>
<%--				<td>
					...
					<td>
						<%if(btnMap.get(menuCode+"-1")!=null) { %>
							<a href="javascript:openDealMon('<%=mon.get("id") %>','<%=mon.get("dealflag") %>','<%=mon.get("dealdesc")!=null?StringEscapeUtils.escapeHtml(mon.get("dealdesc").toString()) :"" %>')" hidefocus="true"><emp:message key="ptjk_jkxq_ssgj_cl" defVal="处理" fileName="ptjk"/></a>
						<%} %>
						&nbsp;&nbsp;<a href="javascript:toHisMon('2900-1100','<%=mon.get("apptype") %>','<%=monName %>')" hidefocus="true"><emp:message key="ptjk_jkxq_ssgj_ls" defVal="历史" fileName="ptjk"/></a><br/>
					</td>
				</tr>
				<%
						}
				...
				%>
				<tr><td align="center" colspan="16"><emp:message key="ptjk_common_wjl" defVal="无记录" fileName="ptjk"/></td></tr>
				<%} %>
				</tbody>
				<tfoot>
					<tr>
		...
		<div id="dealMon" title="<emp:message key='ptjk_jkxq_ssgj_gjcl' defVal='告警处理' fileName='ptjk'/>"  style="padding:10px;padding-top:20px;display:none">
			...
			<table class="deal_tab">
				<tr>
					...
					<td>
						<label>
						<input type="radio" name="dealflag" value="1"> 
				...
				<tr>
					...
					<td>
						<label>
						<input type="radio" name="dealflag" value="2"> 
				...
				<tr>
					...
					<td>
						<label>
						<input type="radio" name="dealflag" value="3"> 
				...
				<tr>
					...
					<td>
						<textarea style="width: 300px;height: 100px;" name="dealdesc" id="dealdesc" onkeyup="getFontNumber($(this))" onblur="getFontNumber($(this))"></textarea>
					</td>
				...
				<td>&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<td>
					<font  style="line-height:12px;float:left;padding-top:5px;color: #656565;"><b id="strlen">0</b><b>/150</b></font>
					</td>
				</tr>
			</table>
