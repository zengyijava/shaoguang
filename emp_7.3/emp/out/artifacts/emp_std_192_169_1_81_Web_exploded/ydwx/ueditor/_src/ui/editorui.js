...
//        'Imageleft', 'ImageRight', 'ImageChar',
    ...
    editorui.ClearDoc = function(editor, title){
        ...
        var ui = new editorui.Button({
            ...
            title: title,
            onclick: function (){
                if(confirm(getJsLocaleMessage("ydwx","ydwx_wxbj_163"))){
    ...
    var dialogNoCloseCmds = ['InsertInteraction'];
    
    k = dialogNoCloseCmds.length;
    while (k --) {
        cmd = dialogNoCloseCmds[k];
        editorui[cmd] = function (cmd){
            cmd = cmd.toLowerCase();
            return function (editor, iframeUrl, title){
                iframeUrl = iframeUrl || editor.options.iframeUrlMap[cmd.toLowerCase()] || 'about:blank';
    ...
    var actDialogCmds = ['Actividata'];
    
    k = actDialogCmds.length;
    while (k --) {
        cmd = actDialogCmds[k];
        editorui[cmd] = function (cmd){
            cmd = cmd.toLowerCase();
    ...
    var dialogCmds = ['Anchor','Link','InsertDownfile', 'InsertImage', 'Map', 'GMap', 'InsertVideo','TableSuper','HighlightCode'];
    ...
    editorui.InsertUnorderedList = function (editor, title){
        ...
        var ui = new editorui.MenuButton({
            ...
            items:
                [{
                    label: '○ '+getJsLocaleMessage("ydwx","ydwx_wxbj_164"),
                    value: 'circle',
                    onclick : _onMenuClick
                },{
                    label: '● '+getJsLocaleMessage("ydwx","ydwx_wxbj_165"),
                    value: 'disc',
                    onclick : _onMenuClick
                },{
                    label: '■ '+getJsLocaleMessage("ydwx","ydwx_wxbj_166"),
                    value: 'square',
                    onclick : _onMenuClick
                }],
            onbuttonclick: function (){
