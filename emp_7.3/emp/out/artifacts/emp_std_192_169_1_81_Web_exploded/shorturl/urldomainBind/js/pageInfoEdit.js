...
function goPage(i){
	...
	//设置为不需要重新查分页信息
	document.forms['form5'].elements["needNewData"].value =2;
	if(page==null || page ===""){
		page_loading();
		document.forms['form5'].elements["pageIndex"].value =1;
		//document.forms['form5'].submit();
        var corpid = $("#corpid").val();
        window.location.href="surl_bind.htm?method=toEdit&pageIndex="+page+"&pageSize="+size+"&corpid="+corpid;
		return;
	}
	if(page-0>pagetotal){
		/*输入页数大于最大页数！*/
		alert(getJsLocaleMessage("common","common_js_pageInfo_3"));
		document.forms['form5'].elements["pageIndex"].value="";
		return;
	}
	...
	page_loading();
	document.forms['form5'].elements["pageIndex"].value=page;
...
function jumpSure()
{
	...
	if(currSize != size)
	{
		...
		document.forms['form5'].elements["needNewData"].value = 1;
	}
	else
	{
		//设置为不需要重新查分页信息
	...
	// document.forms['pageForm'].submit();
    var corpid = $("#corpid").val();
    window.location.href="surl_bind.htm?method=toEdit&pageIndex="+pageIndex+"&pageSize="+pageSize+"&corpid="+corpid;
}
var ie4;
var firefox;
if (document.all){
...
function showPageSize(){
    ...
    $("#setupNum").append("<input  onkeyup=\"value=value.replace(/[^\\d]/g,\'\')\"  onkeypress='return checkPageIndex(event,this);'  maxlength=\"3\" type=\"text\" id='pageSize' name=\"pageSize\" value=\""+pageSize+"\"/>"+getJsLocaleMessage("common","common_js_pageInfo_9"));
    // 显示“共几页，几条”
    $("#AllPage").html("");
    /*共 页/共 条*/
...
function init(initSelect){  //初始化分页
	...
	$(".page_size_unselect").live("click",function(){
		...
		if(size != currSize) {
			...
			//document.forms['pageForm'].submit();
            var corpid = $("#corpid").val();
            window.location.href="surl_bind.htm?method=toEdit&pageIndex=1&pageSize="+size+"&corpid="+corpid;
		} else {
