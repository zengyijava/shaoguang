...
function goPage(i){
	...
	//设置为不需要重新查分页信息
	document.forms['form6'].elements["needNewData"].value =2;
	if(page==null || page ===""){
		page_loading();
		document.forms['form6'].elements["pageIndex"].value =1;
		//document.forms['form6'].submit();
        var corpid = $("#corpid").val();
        window.location.href="surl_bind.htm?method=see&pageIndex=1&pageSize="+size+"&id="+corpid;
		return;
	}
	if(page-0>pagetotal){
		/*输入页数大于最大页数！*/
		alert(getJsLocaleMessage("common","common_js_pageInfo_3"));
		document.forms['form6'].elements["pageIndex"].value="";
		return;
	}
	...
	page_loading();
	document.forms['form6'].elements["pageIndex"].value=page;
	//document.forms['pageForm'].submit();
    var corpid = $("#corpid").val();
    window.location.href="surl_bind.htm?method=see&pageIndex="+page+"&pageSize="+size+"&id="+corpid;
}
...
function jumpSure()
{
	...
	if(currSize != size)
	{
		...
		document.forms['form6'].elements["needNewData"].value = 1;
	}
	else
	{
		//设置为不需要重新查分页信息
