...
function goPage(i){
	...
	if(page==null || page ===""){
        ...
        var addcorpCode;
        if(CstlyeSkin.indexOf("frame4.0") !== -1) {
            addcorpCode = $(".c_input").val();
        }else {
            addcorpCode = $("#addcorpCode").val();
		}
        window.location.href="surl_bind.htm?method=toAdd&pageIndex="+page+"&pageSize="+size+"&add="+addcorpCode;
		return;
	}
	if(page-0>pagetotal){
		/*输入页数大于最大页数！*/
...
function jumpSure()
{
    ...
    window.location.href="surl_bind.htm?method=toAdd&pageIndex="+pageIndex+"&pageSize="+pageSize+"&add="+addcorpCode;
}
var ie4;
var firefox;
if (document.all){
...
function init(initSelect){  //初始化分页
	...
	$(".page_size_unselect").live("click",function(){
		...
		if(size != currSize) {
            ...
            window.location.href="surl_bind.htm?method=toAdd&pageIndex=1&pageSize="+size+"&add="+addcorpCode;
		} else {
