	   ...
	   function moveLeft() {
		...
		function getTypeStr(etype,tempVal)
		{
			if(etype == 1 || etype=="1")
			{
				...
				return getJsLocaleMessage('group','group_js_cliAddMember_client') ; //"客户";
			}
			if(etype == 2 || etype=="2")
			{
    ...
    $("input#epname").removeAttr("disabled");
		...
		/*	if(){
			...
			if(name.length>0 && outSpecialChar(name)){
				...
				//alert("请输入手机号码") ;
				alert(getJsLocaleMessage('group','group_js_sysChooseInfo_enterphone'));
				$("#adPhone").focus();
				zhihui = 0;
			}
			...
			}else
			{
			 ...
			 if(fileArray.toString().indexOf(fileName)>-1)
			 {
				...
				if (filetype != "txt" && filetype != "xls"&& filetype != "xlsx") {
					//alert("上传文件格式错误，请选择txt或xls或xlsx格式的文件。");
             ...
             EmpExecutionContext.log('文件['+fileName+']上传开始','','<客户群组上传>');
			 $.ajaxFileUpload ({ 
			    url: ipathh+'/grp_cliGroupManage.htm?method=upload', //处理上传文件的服务端 
			    ...
			    success: function (data) {
                 ...
                 complete:function(){
                     EmpExecutionContext.log('文件['+fileName+']上传结束','','<客户群组上传>');
                 }
			  });
		 }
		...
		// 处理按纽上一页  
			function prePage(){
				...
				//处理机构是否重复       isDep  1表示是员工机构      2表示是客户机构     3表示是群组
					$.post(path+"/grp_cliGroupManage.htm", {method:methodName,lguserid:lguserid, depId:depId,udgId:tempVal,epname:epname,lgcorpcode:lgcorpcode,pageIndex:pageIndex,type:type}, function(result){
						$("#pagecode").empty();
						if(result != "" ){
							// 获取分页信息 
							var arr = result.substring(0,result.indexOf("#")).split(",");
