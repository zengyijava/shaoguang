...
{	
	...
	$.post("cwc_focusrep.htm",
			{method:"update",lgcorpcode:$("#lgcorpcode").val(),msgText:msgText,AId: AId,title:title,tid:tid,OpType:"add",userid:$("#lguserid").val()},
	    	function(result){
    	    	...
    	    	if(result=="success")
	    		{
		    		...
		    		window.location.href=$("#pathUrl").val()+"/cwc_focusrep.htm?method=find&lguserid="
    	    	...
    	    	$("input[type='button']").attr("disabled","disabled");
    	    	$("#tid").val("");
    	    	window.location.href=$("#pathUrl").val() + "/cwc_focusrep.htm?method=find&lguserid="+$('#lguserid').val()+"&lgcorpcode="+$('#lgcorpcode').val();
	});
}
