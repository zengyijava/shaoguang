$(document).ready(function() {
  ...
  function judeitemtitle(){
	  ...
	  var s = itemIds.split(",");
	  if(s.length<3){
		  alert(getJsLocaleMessage("weix","weix_qywx_hfgl_text_8"));
		  return false;
	  }
	  for(var i in s){
		  if(!isnullstr(s[i])){
			  if(isnullstr($("#item-title"+s[i]).val())){
				  alert(getJsLocaleMessage("weix","weix_qywx_hfgl_text_9")+s[i]+getJsLocaleMessage("weix","weix_qywx_hfgl_text_10"));
				  return false;
			  }else{
				 if($("#item-title"+s[i]).val().length>32){
					 alert(getJsLocaleMessage("weix","weix_qywx_hfgl_text_11") + s[i] + getJsLocaleMessage("weix","weix_qywx_hfgl_text_12"));
					return false;
				 }
			 }
			  if(isnullstr($("#item-url"+s[i]).val())){
				  alert(getJsLocaleMessage("weix","weix_qywx_hfgl_text_13")+s[i]+getJsLocaleMessage("weix","weix_qywx_hfgl_text_14"));
				  return false;
			  }
			  if(!isnullstr($("#item-content"+s[i]).val())){
				  if($("#item-content"+s[i]).val().length>1000){
					  alert(getJsLocaleMessage("weix","weix_qywx_hfgl_text_15")+s[i]+getJsLocaleMessage("weix","weix_qywx_hfgl_text_16"));
					  return false;
				  }	  
			  }
			  if(!isnullstr($("#item-link"+s[i]).val())){
				  if($("#item-link"+s[i]).val().length>50){
					  alert(getJsLocaleMessage("weix","weix_qywx_hfgl_text_15")+s[i]+getJsLocaleMessage("weix","weix_qywx_hfgl_text_17"));
					  return false;
				  }	else{
					  var pattern=/http(s)?:\/\/([\w-]+\.)+[\w-]+([\w-]*)?/;
					  if(!pattern.test($("#item-link"+s[i]).val())){
						  alert(getJsLocaleMessage("weix","weix_qywx_hfgl_text_15")+s[i]+getJsLocaleMessage("weix","weix_qywx_hfgl_text_18"));
						  return false;
					  }
  ...
  function clearitemtitle(){
	  ...
	  for(var i in s){
    	   ...
    	   if(s[i] == 1){
			    ...
			    $("#appmsgItem1").find(".i-title").html(getJsLocaleMessage("weix","weix_qywx_hfgl_text_19"));
  ...
  function uploadImage(obj) {
    	...
    	if(imgtem.length<1){
	    	alert(getJsLocaleMessage("weix","weix_qywx_hfgl_text_20"));
	    	return ;	
	    }else{
