...
for i in range(len(num_list)):
	for j in range(len(x)): 
		x[j] = x[j] + width  
	if (i+1)==a :
		plt.bar(x, num_list[i], width=width, label=lable_list[i],color = colors[i],tick_label = name_list,align="center") 
	else :
		plt.bar(x, num_list[i], width=width, label=lable_list[i],color = colors[i],align="center")
