...
function rtime() {
    ...
    WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:sendtime||min,maxDate:max});
}
...
function stime(){
    ...
    var min = "1900-01-01 00:00:00";
    WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:min,maxDate:recvtime||max});
}
...
function showTitle(obj) {
    var $title = $(obj);
    $('#showTitle').html($title.attr("title"));
    $('#showTitle').dialog({
        autoOpen: false,
        width:300,
        height:300
    });
    $('#showTitle').dialog("open");
}
