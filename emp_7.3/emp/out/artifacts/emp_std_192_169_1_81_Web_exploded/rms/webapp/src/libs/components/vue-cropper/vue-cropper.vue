...
export default {
  data () {
    return {
      ...
      cropChangeX: 0,
      cropChangeY: 0,
      cropOffsertX: this.cropLeft,
      cropOffsertY: this.cropTop,
      // 支持的滚动事件
      support: '',
      // 移动端手指缩放
      touches: [],
  ...
  watch: {
    ...
    cropLeft (val) {
      this.cropOffsertX = val
    },
    cropTop (val) {
      this.cropOffsertY = val
    },
    cropWidth (val) {
      this.cropW = val
    },
  ...
  methods: {
    ...
    moveCrop (e, isMove) {
      ...
      this.$nextTick(() => {
        ...
        if (this.centerBox) {
          let axis = this.getImgAxis();
          // 横坐标判断
          if (parseInt(cx) < axis.x1) {
            cx = axis.x1 + 1;
          } else if (parseInt(cx) + this.cropW > axis.x2) {
            cx = axis.x2 - this.cropW;
          }

          // 纵坐标纵轴
          if (parseInt(cy) < axis.y1) {
            cy = axis.y1;
          } else if (parseInt(cy) + this.cropH > axis.y2) {
            cy = (axis.y2 - this.cropH - 1) < 0 ? 0 : axis.y2 - this.cropH - 1;
          }
        }
    ...
    showPreview () {
      ...
      obj.h = this.cropH;
      obj.url = this.imgs
      obj.left = this.cropOffsertX
      obj.top = this.cropOffsertY
    ...
    goAutoCrop (cw, ch) {
      ...
      // 截图框默认大小
      // 如果为0 那么计算容器大小 默认为80%
      var w = cw || this.autoCropWidth;
      var h = ch || this.autoCropHeight;
      if (w === 0 || h === 0) {
        w = maxWidth * 0.8;
        h = maxHeight * 0.8;
      }
    ...
    changeCrop (w, h) {
      ...
      this.cropH = h;
      // 居中走一走
      if (this.cropOffsertX === 0) {
        this.cropOffsertX = (this.w - w) / 2;
      }
      if (this.cropOffsertY === 0) {
