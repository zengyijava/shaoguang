<template>
  <div class="link-jump">
    <el-form-item label="请选择按钮操作">
      <el-radio-group size="small" v-model="element.action">
        <el-radio-button label="2">打开链接</el-radio-button>
        <el-radio-button label="4">APP包名</el-radio-button>
        <el-radio-button label="7">打开快应用</el-radio-button>
        <el-radio-button label="5">打开地图</el-radio-button>
      </el-radio-group>
    </el-form-item>
    <el-form v-if="element.action==='5'" label-position="left">
      <el-form-item class="city-show" v-if="addr" label="城市" label-width="40px">
        <span>{{ currentCity }}</span>
      </el-form-item>
      <el-form-item class="city-show" v-if="addr" label="地址" label-width="40px">
        <span>{{ addr + " " + title }}</span>
      </el-form-item>
      <el-form-item align="right">
        <el-button type="primary" size="small" @click="dialogTableVisible = true">选择地理位置</el-button>
      </el-form-item>
    </el-form>
    <section v-else>
      <el-form-item v-if="element.action === '2'" label="链接地址：">
        <el-input size="small" placeholder="如http://montnets.com" v-model="element.url"></el-input>
      </el-form-item>
      <el-form-item v-else-if="element.action === '4'" label="APP地址：">
        <el-input size="small" placeholder="如com.tencent.mobileqq" v-model="element.pkg"></el-input>
      </el-form-item>
      <div v-else-if="element.action === '7'">
        <el-form-item label="包名" label-width="40px">
          <el-input size="small" placeholder="如com.tencent.mobileqq" v-model="element.pkg"></el-input>
        </el-form-item>
        <el-form-item label="路径" label-width="40px">
          <el-input size="small" placeholder="如user/info" v-model="element.path"></el-input>
        </el-form-item>
        <el-form-item label="参数" label-width="40px">
          <el-input size="small" placeholder="如param1=value1" v-model="element.param"></el-input>
        </el-form-item>
      </div>
    </section>
    <el-dialog class="map-dialog" title="地理位置选择" top="10vh" :visible.sync="dialogTableVisible" @open="initBaiduMap" append-to-body>
      <div class="dialog-cont clearfix">
        <div class="search-cont">
          <el-input placeholder="请输入内容" v-model="addressVal" size="small">
            <el-button slot="append" type="primary" icon="el-icon-search" @click="searchAddress"></el-button>
          </el-input>
          <div id="search-result" class="search-result">
            <div v-for="(item, index, key) in searchAddressArr" :key="key" :class="[{'active': index === activeIndex},'search-li', 'clearfix']" @click="getCurrentPoint(item, index)">
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" size="small" @click="confirmAddress">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    // 设置地图标点文字显示
    setLableText: function (point, text) {
      let _labelOpts = {
        position: point,
    ...
    // 获取当前点击搜索结果的经纬度
    getCurrentPoint: function (data, index) {
