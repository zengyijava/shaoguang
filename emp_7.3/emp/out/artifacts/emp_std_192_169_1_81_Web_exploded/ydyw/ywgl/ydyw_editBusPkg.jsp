...
<%@page import="com.montnets.emp.entity.ydyw.LfBusPackage"%>
...
LfBusPackage pkgEntity = (LfBusPackage)request.getAttribute("pkgList");
List<DynaBean> haveBusList = (List<DynaBean>)request.getAttribute("haveBusList");
...
<html>
	...
	<input type="hidden" id="pathUrl" value="<%=path %>"/>
				...
				<center>
					<div style="text-align: left;padding-left: 30px;padding-top: 15px;">
						<form action="<%=path %>/ydyw_busPkgMgr.htm?method=update" method="post" name="busPkgform" id="busPkgform">
							...
							<input type="hidden" id="pkgId" name="pkgId" value="<%=null==pkgEntity?"":pkgEntity.getPackageId() %>"/>
							<table cellpadding="0" cellspacing="0" border="0" class="contTable">
								<tr>
									...
									<td>
										<input type="text" class="input_bd div_bd wth1" id="busPkgName" name="busPkgName" maxlength="25" value="<%=null==pkgEntity?"":pkgEntity.getPackageName() %>"/>
								...
								<tr>
									...
									<td>
										<%=null==pkgEntity?"":pkgEntity.getPackageCode() %>
									</td>
								</tr>
								<tr><td colspan="2" style="height: 15px;"></td></tr>								
								<tr>
									...
									<td style="background-color: #f0f2f9;padding: 10px 20px 20px 20px;border:solid 1px #c8daf3;">
										<input type="text" class="input_bd div_bd wth2" id="busSearStr" name="busSearStr" style="float:left;height: 21px;margin-top: 2px;"/>
										<a onclick="searchName();" class="searIcon"><img id="searchIcon" src="<%=skin %>/images/query.png" border="0"/></a>
										<span style="margin-left:170px;color: #333433;"><emp:message key="ydyw_ywgl_ywbgl_text_7" fileName="ydyw" defVal="已选择业务数："></emp:message><label id="manCount"><%=null==haveBusList?"0":haveBusList.size() %></label></span>
											...
											<tr>
												...
												<td>
													<div id="rightDiv"   class="div_bd" >
														...
														<% 
														if(haveBusList!=null&&haveBusList.size()>0){
														for(int j=0;j<haveBusList.size();j++){
															DynaBean bean=haveBusList.get(j);
														%>
															<option value="<%=bean.get("bus_code")%>"><%=bean.get("bus_name")%>(<%=bean.get("bus_code")%>)</option>
														<%}
														}
														%>												
														</select>
														...
														<% 
														if(haveBusList!=null&&haveBusList.size()>0){
														for(int k=0;k<haveBusList.size();k++){
															DynaBean bean=haveBusList.get(k);
														%>
															<li title="<%=bean.get("bus_name")%>(<%=bean.get("bus_code")%>)" dataval="<%=bean.get("bus_code")%>">
																<%=bean.get("bus_name")%>(<%=bean.get("bus_code")%>)<%=null!=bean.get("state") && "1".equals(bean.get("state").toString())?"<span style=\"color:red;\"> 已禁用</span>":"" %>
															</li>
														<%}
														}
														%>											
		...
		<script type="text/javascript" src="<%=iPath%>/js/editBusPkg.js?V=<%=StaticValue.getJspImpVersion() %>"></script>
