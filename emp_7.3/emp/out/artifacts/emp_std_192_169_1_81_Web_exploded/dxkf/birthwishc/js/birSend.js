...
function doOk() {
    ...
    $(window.frames['flowFrame'].document).find("#right option").each(function () {
        if(skin.indexOf("frame4.0") > -1){
            if($(this).attr("isdep") === "2"){
                //机构
                clientDepName += $(this).html() + ",";
            }else if($(this).attr("isdep") === "5"){
                //客户
    ...
    $("#selectWorkerButton").val(getJsLocaleMessage('dxkf','dxkf_srzf_birSend_xgcy'));
}
...
function doNo(){
    ...
    $(window.frames['flowFrame'].document).find("#cliDepIdsStrs").empty();
    $(window.frames['flowFrame'].document).find("#clientIds").empty();
...
function setDefault()
{
	if(confirm(getJsLocaleMessage('dxkf','dxkf_dxqf_page_setdefault'))) {
		...
		$.post("bir_birthdaySendClient.htm?method=setDefault", {
			lguserid: lguserid,
