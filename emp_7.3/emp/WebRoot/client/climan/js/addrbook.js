var pageText = {
    pageSkipText: getJsLocaleMessage("common","common_page_skip"),
    pageText: getJsLocaleMessage("common","common_page_page"),
    pageRowpageText: getJsLocaleMessage("common","common_page_rowpage")
};
...
//	var lguserid=$("#lguserid").val();
	...
	$('#bookInfo').load(path+'/cli_addrBook.htm?method=getTable',
	...
	if(confirm(getJsLocaleMessage("common","common_js_addrBook_2"))){
	    ...
	    $.ajax({
	        ...
	        success: function(result){
				...
				if (result>0) {
					//alert("删除选中记录成功！");
					alert(getJsLocaleMessage('client','client_js_addrbook_deletesuc'));
					submitForm();
				}else if(result<=0){
...
function toSysuser()
{
	...
	if(count != "1"){
		//alert("请选择一个员工进行操作！");
		alert(getJsLocaleMessage('client','client_js_addrbook_selectuseropt'));
		return;
	}
	else
	{
	...
	function init(){  //初始化分页
		...
		$pageWrapper.append('<div id="page_new"><ul class="page_ul">' +
				...
				'<li class="normal_text_li">' +
					'<span class="normal_text">'+pageText.pageSkipText+'</span>' +
				'</li>' +
				'<li class="page_index_input page_li_hb">' +
					'<input type="text" id="page_input_value">' +
				'</li>' +
				'<li class="normal_text_li">' +
					'<span class="normal_text">'+pageText.pageText+'</span>' +
				'</li></ul></div>');
		...
		for(var i = 0; i < initSelect.length; i++) {
			...
			if(i == 0) {
				...
				'<span class="page_select_text">' + initSelect[i] + pageText.pageRowpageText+'</span></div>';
