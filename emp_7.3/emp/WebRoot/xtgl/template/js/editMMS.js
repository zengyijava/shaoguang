...
setting = {
	async : true,
	asyncUrl :"tem_mmsTemplate.htm?method=getMatelTree", //获取节点数据的URL地址
	isSimpleData: true,
	rootPID : 0,
	treeNodeKey: "id",
...
function zTreeOnClick(event, treeId, treeNode) {
	if (treeNode && !treeNode.isParent) {
		...
		if ("AMR" != type &&!"MID" != type && "MIDI" != type){
			...
			result = "<img src=\'"+$("#skin").val+"/images/hasMusic.png"+"\'/>"+name+"(ID:"+id+")";
		}
		 $("#matList").empty();
...
$("input[name='templateType'][value="+dsflag+"]").attr("checked","checked");
if(dsflag == 0){
	$("#paramTxt").hide();
}else{
	$("#paramTxt").show();
...
var msg = $("#tmMsg").val();
var lgcorpcode = $("#lgcorpcode").val();
$.post("tem_mmsTemplate.htm?method=edit",{tmUrl:msg,lgcorpcode:lgcorpcode},function(result){
