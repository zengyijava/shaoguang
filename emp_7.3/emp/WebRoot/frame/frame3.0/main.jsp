...
//用于提示多少天
String Logindays=(String)session.getAttribute("Logindays");
...
<html>
    ...
    <link rel="shortcut icon" href="<%=path%>/common/img/favicon.ico" mce_href="<%=path%>/common/img/favicon.ico" type="image/x-icon" />
    <link rel="bookmark" href="<%=path%>/common/img/favicon.ico" mce_href="<%=path%>/common/img/favicon.ico" type="image/x-icon"/>
    ...
    <style type="text/css">
    .ui-widget-header{border-bottom:none;border-bottom-left-radius:0;border-bottom-right-radius:0;}
    .fontsel{background:url(img/sel.gif) no-repeat 0 0;}
    .classsic_change .themeSpan{background:url(img/icon_tradition.png) no-repeat 15px 50%;}
    .tradition_change .themeSpan{background:url(img/icon_classic.png) no-repeat 15px 50%;}
    ...
    .classsic_change .fontsel{background:url(img/icon_tradition_sel.png) no-repeat 15px 50%;}
    .tradition_change .fontsel{background:url(img/icon_classic_sel.png) no-repeat 15px 50%;}
    #themeDiv .selected{background:url(img/sel.gif) no-repeat 0 0;}
     #jkBox
    {
    	border:1px solid #ccc;width: 200px;position: fixed;*position:absolute;right: 5px;font-size: 12px;
...
<div id="bdbg_left"></div>
 <div id="loginparams" class="hidden"><%
	 ...
	 .isDepBilling((corpcode == null || "".equals(corpcode.trim())) ? "100001"
	 		: corpcode);//是否启用计费机制
     %><input type="hidden" name="tkn" id="tkn" value="<%=tkn %>"/> </div>
	...
	<div id="wrap" style="height: 100%; border: 0;overflow: hidden;display:block;width:100%;height:100%;position: absolute;z-index: 1;">
		<div style="width:100%;height:70px;position: absolute;z-index: 12;">
			<iframe style="border: 0;" frameborder="0" border=0 height="100%" src="<%=path %>/thirdMenu.htm?method=toTop&time=<%=System.currentTimeMillis() %>&tkn=<%=tkn %>&corpcode=<%=corpcode %>&userid=<%=userid %>" name="topFrame" scrolling="no" id="topFrame" allowtransparency="true" ></iframe>
		</div>
		<div id="top_frame_div">
			<iframe style="border: 0;" frameborder="0" border=0 height="100%" src="<%=path %>/thirdMenu.htm?method=getMonInfo"  scrolling="no"  name="mainFrame" id="mainFrame" allowtransparency="true" ></iframe>
		</div>
	</div>
	...
	<div id="dd" class="floating" style="">
   		<ul id="uChildren" class=""></ul>
   	</div>
   	<iframe id="ifr" class="ifr"></iframe>
   	<%-- 语言切换 --%>
	<%if(("Yes".equals(multiLanguageEnable) && langs != null && !langs.isEmpty()) && langs.size() > 1){%>
	<div id="lang" class="floating" style="">
   		<ul id="uChildren" class="">
			<%
				for(String str : langs){
					String langName = "zh_HK".equals(str)?"English":"zh_TW".equals(str)?"繁體":"简体";
			%>
				<li><a href="#" onclick="javascript:changeLanguage('<%=str%>')"><%=langName%></a></li>
   			<%}%>
   		</ul>
   	</div>
	<%}%>
   	<%-- 信息 --%>
   	<div id="loginMesFloating" class="floating">
   		<ul class="uChildren" id="">
   			<li><a href="#" onclick="doUpdatePass()" id="loginMesFloating-1"><emp:message key="common_personalSetting" defVal="个人设置" fileName="common"></emp:message></a></li>
   			<li><a href="#" onclick="parent.frames['topFrame'].window.upLoad()" id="loginMesFloating-2"><emp:message key="common_helpManual" defVal="帮助手册" fileName="common"></emp:message></a></li>
   			<li><a href="#" onclick="showAbout()" id="loginMesFloating-3"><emp:message key="common_aboutPlatform" defVal="关于平台" fileName="common"></emp:message></a></li>
   			<li><a href="#" onclick="parent.frames['topFrame'].window.logout()" id="loginMesFloating-4">退&nbsp;&nbsp;出</a></li>
   		</ul> 
   	</div>	
   	<%-- 余额 --%>
	<div id="rNumFloating" class="floating1" style="">
  		...
  		<%if (isCharging) 
          {%>
  			<li id="smsYe"><a href="#" ><emp:message key="common_frame2_main_8" defVal="机构短信余额：" fileName="common"></emp:message><span id="ctSms" style="color:red;">0</span></a></li>
  			<li id="mmsYe"><a href="#" ><emp:message key="common_frame2_main_9" defVal="机构彩信余额：" fileName="common"></emp:message><span id="ctMms" style="color:red">0</span></a></li>
  		<%} %>
  			<%if(isShowYe){ %>
  				<li><a href="#" id="checkfee" onclick="parent.frames['topFrame'].window.checkFee()"><emp:message key="common_frame2_main_10" defVal="查看运营商余额" fileName="common"></emp:message></a></li>
		    <%}%>
  		</ul>
   	</div>  
    ...
    <div id="updateinfoDiv" style="height: 315px;">
      ...
      <div id="updatePwDiv" style="display: none;height: 315px;">
      <br/>
   ...
   <div id="themeDiv" class="ui-widget-header" style="width:100%;overflow:hidden;padding:12px 0 0;">
      <div class="simple_change ml23" ><span class="themeSpan" themecode="frame4.0" onclick="getThemeSkin('frame4.0')"><emp:message key="common_simple" defVal="简约" fileName="common"/></span></div>
      <div class="classsic_change ml23 selected" ><span class="themeSpan" themecode="frame3.0" onclick="getThemeSkin('frame3.0')"><emp:message key="common_classic" defVal="经典" fileName="common"></emp:message></span></div>
      <div class="tradition_change" ><span class="themeSpan" themecode="frame2.5" onclick="getThemeSkin('frame2.5')"><emp:message key="common_tradition" defVal="传统" fileName="common"></emp:message></span></div>
    </div>
    <center><div style="width:722px;height:375px;overflow: hidden;padding-top:5px" id="skinlist"></div>
    ...
    <div id="weixArgu" style="display:none;">
     ...
     <div  style="margin:0 auto;width:86px;"><input type="button" value="<emp:message key="common_accept" defVal="接受" fileName="common"></emp:message>" class="btnClass5" id="argeeArg" onclick="$('#mainFrame').contents().find('#cont<%=ViewParams.ACCTMANAGER%>')[0].contentWindow.getApprove();"/></div>
     </div> 
		...
		<div class="inner">
			<div class="toolbar">
				<div class="tool_warn">
					...
					<span class="winMax" onclick="openMonMain()"></span>
					<span class="winClose" onclick="clioseMonDialog()"></span>
				</div>
    ...
    <script type="text/javascript">
	    ...
	    /*End-change of the emp skin*/
        var wxaccount="<%=ViewParams.ACCTMANAGER%>";
	    ...
	    function wei_tips_dialog(txt){
	    	$('#weixArgu').dialog({
				...
				close:function(){
					var aprove = window.parent.frames['mainFrame'].frames['cont'+wxaccount].document.getElementById("approve").value;
					if(aprove!="1"){
						$(window.parent.document).find("#topFrame").contents().find("#top_home").trigger('click');
					}
				}
			});
        ...
        $(function(){
	        ...
	        function getField(obj){
	        	return $(window.document).find(obj).val();
	        }
	        function getLoginparams(obj){
	        	var $pa = $(window.parent.document);
	        	var pahtm = $("#loginparams").children(obj).val();
	        	return pahtm;
	        }
        })
    </script> 
