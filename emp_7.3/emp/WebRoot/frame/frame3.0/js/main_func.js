...
$(document).ready(function () {
    ...
    $('#rNumFloating').hover(function () {
        var $par = $(window.frames["topFrame"].document);
        $par.find("#remainingNum").removeClass("selected");
        $par.find("#remainingNum").addClass("onSys");
        $("#ifr").show();
        $(this).show();
    }, function () {
        var $par = $(window.frames["topFrame"].document);
        var onSys = $par.find("#onSys").val();
        $par.find("#remainingNum").removeClass("onSys");
        $(this).hide();
        $("#ifr").hide();
    });
    //换肤
    $('#lang').hover(function () {
        var $par = $(window.frames["topFrame"].document);
        $par.find("#lg").removeClass("selected");
        $par.find("#lg").addClass("onSys");
        $("#ifr").show();
        $(this).show();
    }, function () {
        var $par = $(window.frames["topFrame"].document);
        var onSys = $par.find("#onSys").val();
        $par.find("#lg").removeClass("onSys");
        $(this).hide();
        $("#ifr").hide();
    });

    $('#loginMesFloating').hover(function () {
        var $par = $(window.frames["topFrame"].document);
        $par.find("#loginMes").removeClass("selected");
        $par.find("#loginMes").addClass("onSys");
        $("#ifr").show();
        $(this).show();
    }, function () {
        var $par = $(window.frames["topFrame"].document);
        var onSys = $par.find("#onSys").val();
        $par.find("#loginMes").removeClass("onSys");
        $(this).hide();
        $("#ifr").hide();
    });
    ...
    //阻止在IE下 frame3.0下 切换top也大模块菜单时 触发onbeforeunload事件 导致页面出现加载层
    if (document.all) {
...
function replaceSkin($docu, new_skin) {
    ...
    for (var i = 0; i < $hcss.length; i = i + 1) {
        ...
        if (csshref.indexOf(old_skin_css) > -1) {
            csshref = csshref.replace(old_skin_css, new_skin_css);
            $hcss.eq(i).attr("href", csshref);
        } else if (csshref.indexOf("index-v3")) {   // 兼容新的查询统计模块, 并不影响原来的逻辑
            var tmp_old_skin_css = "index-v3-" + old_skin;
            var tmp_new_skin_css = "index-v3-" + new_skin;
            ...
            if (result == "outOfLogin") {
                ...
                if (result != "[]") {
                    ...
                    for (var i = 0; i < monMsgJson.length; i++) {
                        var str = i > 0 ? "class='dl_top'" : "";
                        MonMsgHtml = MonMsgHtml + "<dl onclick='toErrMon(" + monMsgJson[i].id + ")' " + str + " style='cursor: pointer;'><dt>" + monMsgJson[i].evttime + "</dt><dd>" + monMsgJson[i].monname + "：" + monMsgJson[i].msg + "</dd></dl>";
                    }
...
function openNewTabMon(menuCode, url) {
    ...
    if ($lf.contents().find("a[id='ak" + menuCode + "']").length == 0) {
        window.frames['mainFrame'].doOpen('16');
        return;
    }
    ...
    if (url != "") {
        $(window.parent.frames['mainFrame'].document).find("#cont" + menuCode).attr("src", url);
    }
}
