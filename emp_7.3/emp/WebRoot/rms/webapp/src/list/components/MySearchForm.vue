...
export default {
  ...
  computed: {
    ...
    // 审核状态下拉选项
    auditStatusOption () {
      let _auditStatusOptionLabel = this.$t('mySceneSearchText.auditStatusSelect.options')
      let _auditStatusOptionVal = {
        all: {value: ''},
        no: {value: '-1'},
        underway: {value: '3'},
        forbid: {value: '4'},
      ...
      // 为对应的option添加value值
      for (let key in _auditStatusOptionLabel) {
        this.$set(_auditStatusOptionLabel[key], 'value', _auditStatusOptionVal[key].value)
      }

      return _auditStatusOptionLabel
    }
  },
  methods: {
    // 是否显示富信类型筛选
