...
$(document).ready(function() {
	...
	$("#mframes").children().click(function(){
		...
		//$("#curf").html("当前"+$.trim($(this).attr("id"))+"/"+frameid);
		$("#curf").html(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_dq")+$.trim($(this).attr("id"))+"/"+frameid);
	});
	$("#1").hover(function() {
	...
	$("#model").dialog({				
		autoOpen: false,
		//title:"添加图片",
		title:getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_tjtp"),
		height:200,
		width: 310
	});
	//添加声音的dialog
	$("#model2").dialog({				
		autoOpen: false,
		//title:"添加声音",
		title:getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_tjsy"),
		height:200,
		width: 310
	});
	//点击调用素材
	$("#addMat").click(function(){		
		...
		if(focusFrameId == "addFrame"){
			...
			//alert("请选择帧后再调用素材文件！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_qxzzhzdyscwj"));
			return ;
		}
		$("#mmsMat").dialog("open");
	});
	...
	$("#addImg").click(function(){	
		...
		if(focusFrameId == "addFrame"){
			//如果当前选中帧为“添加帧按钮”，则return
			//alert("请选择帧后再上传图片！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_qxzzhzsctp"));
			return ;
		}
		$("#model").dialog("open");
	});
	...
	$("#addMusic").click(function(){		
		...
		if(focusFrameId == "addFrame"){
			//如果当前选中帧为“添加帧按钮”，则return
			//alert("请选择帧后再上传声音文件！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_qxzzhzscsywj"));
			return ;
		}
		$("#model2").dialog("open");
	});
	...
	$("#mscan").click(function(){					
		...
		for (var i=0;i<arr.length;i++)				
		{
		    ...
		    if (data != null && data != "")
		    {
		    	if(data.ImgUrl == "" && data.MusUrl=="" && data.textContent=="")
		    	{
		    		...
		    		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_d")+(parseInt(i)+1)+getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_zwkbzqxg"));
					return;
		    	}
		    	if(data.ImgUrl != "" || data.MusUrl!=""|| data.textContent!="")
		    	{
	   ...
	   var dsFlag = $("input[name:'templateType']:checked").val();
		if(dsFlag == 1 && paramCnt>0)
		{
    		...
    		{
        		//str='参数'+(i+1)+': &nbsp;<input type="text" id="pa'+i+'" value=""/></br><br/>';
        		str=getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_cs")+(i+1)+': &nbsp;<input type="text" id="pa'+i+'" value=""/></br><br/>';
        		strs=strs+str;
    		}
...
function check(type){		
	...
	if (type == "chooseImg"){
		...
		{
			//alert("请选择要上传的图片文件！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_qxzyscdtpwj"));
			flag = false;
		}else{
		...
		{
			//alert("请选择要上传的声音文件！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_qxzyscdsywj"));
			flag = false;
		}
...
function doUp(type)	{
	...
	if (check(type)){		//这里判断上传的 图片或者声音是否合法	
		if(type == "chooseImg"){
			$.ajaxFileUpload ({ 
			    url:pathUrl+'/mbgl_mytemplate.htm?method=upload&lgcorpcode='+lgcorpcode+'&lguserid='+lguserid, //处理上传文件的服务端 
			    ...
			    success: function (data) { 
				    if(data != null && data.url == "false")
				    {
				    	...
				    	alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_scdwjdxbncg80kb"));
				    	return;
				    }
			      ...
			      $.post('common.htm?method=frontLog',{
				    //info:'彩信上传图片，表单流上传失败。userId:'+ lguserid +'，lgcorpcode:'+ lgcorpcode
				    info:getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_cxsctp")+'userId:'+ lguserid +'，lgcorpcode:'+ lgcorpcode
			      });
				  //alert("上传失败。");
				  alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_scsbai"));
				  return;
			  	}
			  });
		}else if(type == "chooseMusic"){
			$.ajaxFileUpload ({ 
			    ...
			    success: function (data) { 
				 if(data != null && data.url == "false")
			  	...
			  	error: function (data) { 
			      $.post('common.htm?method=frontLog',{
				    //info:'彩信上传声音，表单流上传失败。userId:'+ lguserid +'，lgcorpcode:'+ lgcorpcode
				    info:getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_cxscsy")+'userId:'+ lguserid +'，lgcorpcode:'+ lgcorpcode
			      });
				  //alert("上传失败。");
				  alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_scsbai"));
				  return;
...
function setMus(url,size)
{
   ...
   // $("#soundPlay").html("<embed id='swfEmbed' src='"+url+"' quality='high' pluginspage='http://www.macromedia.com/go/getflashplayer' type='application/x-shockwave-flash' width='70' height='40'></embed>");
    if(url.length>0){
   	 var soundtype = $.trim(url.substring(url.lastIndexOf(".")+1));
   	 if(soundtype == "amr" || soundtype == "AMR"){
   	     //$("#soundPlay").html("<object classid='clsid:22D6F312-B0F6-11D0-94AB-0080C74C7E95'  width='70' height='40'><param name='FileName' value='"+url+"'/></object>");
	     $("#soundPlay").html("<embed src='"+url+"' style='HEIGHT: 45px; WIDTH: 250px' type='audio/mpeg' AUTOSTART='1' loop='0'/>");
   	 }else{
	  	   	 //$("#soundPlay").html("<embed id='swfEmbed' src='"+url+"' quality='high' pluginspage='http://www.macromedia.com/go/getflashplayer' type='application/x-shockwave-flash' width='70' height='40'></embed>");
		 	$("#soundPlay").html("<object classid='clsid:22D6F312-B0F6-11D0-94AB-0080C74C7E95'  width='250' height='45' type=application/x-oleobject><param name='FileName' value='"+url+"'/><param name='AutoStart' value='0'/></object>");
   	 }
	 }
...
function getToatalSize(size,method)					
{
	...
	{
		//alert("当前彩信已经超过最大值80KB了，请删除一些内容！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_dqcxyjcgzdz80kbl"));
	}
...
function delatt(d)				
{
	...
	if (sourceUrl!="")
	{
		$.post(pathUrl+"/mbgl_mytemplate.htm?method=delSource",{sourceUrl:sourceUrl},function(result){
	...
	else
	{
		...
		else
		{
		  //  $("#soundPlay").html("<embed id='swfEmbed' src='"+newContent.MusUrl+"' quality='high' pluginspage='http://www.macromedia.com/go/getflashplayer' type='application/x-shockwave-flash' width='70' height='40'></embed>");
			var soundurl = newContent.MusUrl;
	    	if(soundurl.length>0){
	  	    	 var soundtype = $.trim(soundurl.substring(soundurl.lastIndexOf(".")+1));
	  	    	 if(soundtype == "amr" || soundtype == "AMR"){
	  	    		$("#soundPlay").html("<object classid='clsid:22D6F312-B0F6-11D0-94AB-0080C74C7E95'  width='70' height='40'><param name='FileName' value='"+soundurl+"'/></object>");
	  		     }else{
	  		    	$("#soundPlay").html("<embed id='swfEmbed' src='"+soundurl+"' quality='high' pluginspage='http://www.macromedia.com/go/getflashplayer' type='application/x-shockwave-flash' width='70' height='40'></embed>");
	  			 }
	  		 }
		    $("#delMus").show();
		}
...
function addFrame()    
{
    ...
    {
    	//alert("不能超过15帧！");
    	alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_bncg15z"));
    	return;
    }
    else
    {
    	...
    	$("#curf").html(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_dq")+$.trim(id)+"/"+$.trim((parseInt(frameid)+1)+""));
...
function delFrame(obj)				
{
	...
	if(id == 1){
		//alert("第一帧不允许删除");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_dyzbxysc"));
		return;
    }
	//获取贞的数量
	var frameid = $("#framesize").attr("value");					
	if(frameid<1){
		//alert("没有可供删除的帧！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mykgscdz"));
		return;
	}
	//if(confirm("确定要删除当前帧吗？"))
	if(confirm(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_qdyscdqzm")))
	{
...
function doSubmit()
{
	...
	if(acc=="false" && st=="1")
	{
		...
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mycxfszh"));
		return;
	}
	...
	{
		//alert("模板名称不能为空！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbmcbnwk"));
		return;
	}
	else if(/(\<{1,})|(\>{1,})|(\'{1,})|(\"{1,})|(\\{1,})/.test(mmsTitle)){
		//alert("模板名称中包含不允许的特殊字符(\",\',\<,\>,\\)！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbmczbhbyxdtszf"));
	    return;
	}
	...
	if (mmsCode == "") {
		//alert("模板编号不能为空！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbbhbnwk"));
		return;
	}else if(escape(mmsCode).indexOf("%u")!=-1){
		//alert("模板编号不能包含汉字！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbbhbnbhhz"));
		return;   
	}else if(pattern.test(mmsCode)){
		//alert("模板编号不能包含特殊字符！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbbhbnbhtszf"));
		return;   
	}else if(mmsCode.indexOf(' ')>-1){
		//alert("模板编号不能包含空格！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbbhbnbhkg"));
		return;   
	}
	...
	{
		//alert("模板内容不能为空！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbnrbnwk"));
		return;
	}
	if(parseFloat(si)-parseFloat(80)>0.00)
	{
		//alert("模板内容不能大于80KB！")
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbnrbndy80kb"))
		return;
	}
    else
	{
    	...
    	var lguserid = $("#lguserid").val();
    	var lgcorpcode = $("#lgcorpcode").val();
    	$.post(pathUrl+"/mbgl_mytemplate.htm?method=checkBadWord",{content:contents,lgcorpcode:lgcorpcode,templateType:templateType},function(result){
    		if (result != null && result == "")
    		{
    			$.post(pathUrl+"/mbgl_mytemplate.htm?method=update",{tmId:tmId,mt:mmsTitle,mmsCode:mmsCode,hiddenCode:hiddenCode,cont:contents,s:st,opType:opType,
    				templateType:templateType,lguserid:lguserid,lgcorpcode:lgcorpcode},function(result){
    				if(result != null && result == "overSize"){
    					//alert("tms文件不能大于80KB！");
    					alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_tmswjbndy80kb"));
    					...
    					//alert("保存草稿成功！");
    					alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_bccgcg"));
    					location.href = pathUrl+"/mbgl_mytemplate.htm?lgguid="+lgguid;
    				}else if(result != null && result == "true")
    				{
    					if(opType == "add"){
    						//alert("模板新建成功！请等待审核！");
    						alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbxjcg"));
        					//location.href = location;
    					}else if(opType == "edit"){
    						//alert("模板编辑成功！");
    						alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbbjcg"));
    					}else{
    						//alert("模板复制成功！");
    						alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbfzcg"));
    					}
    					location.href = pathUrl+"/mbgl_mytemplate.htm?lgguid="+lgguid;
    				}
    				else if(result != null && result == "false")
    				{
    					...
    					$("#cancel").attr("disabled",false);
    		    		return;
    				}else if(result != null && result == "repeat")
    				{
    					//alert("模板编号重复！");
    					alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbbhcf"));
    					$("#save").attr("disabled",false);
    					$("#mscan").attr("disabled",false);
    					$("#cancel").attr("disabled",false);
    		    		return;
    		...
    		{
    			//alert("内容中动态参数填写不合法!");
    			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_nrzdtcstxbhf"));
    			$("#save").attr("disabled",false);
				$("#mscan").attr("disabled",false);
				$("#cancel").attr("disabled",false);
    			return;
    		...
    		{
    			//alert("请输入参数！");
    			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_qsrcs"));
    			$("#save").attr("disabled",false);
				$("#mscan").attr("disabled",false);
				$("#cancel").attr("disabled",false);
    	    	return ;
    		...
    		{
    			//alert("模板参数最多20个！");
    			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbcszd20g"));
    			$("#save").attr("disabled",false);
				$("#mscan").attr("disabled",false);
				$("#cancel").attr("disabled",false);
    	    	return ;
    		...
    		else if(result != null && result != "")
    		{
    			...
    			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_nrbhgjz")+result);
    			$("#save").attr("disabled",false);
				$("#mscan").attr("disabled",false);
				$("#cancel").attr("disabled",false);
	    		return;
...
//		  	    	 }
		    ...
		    //上一帧，下一帧 
		    //$("#nextpage").html("&nbsp;&nbsp;&nbsp;<a href='#' style='TEXT-DECORATION:none;color:blue;' onclick='javascript:advanp("+index+");return false'>上一帧</a>&nbsp;&nbsp;<a id='next' href='#' style='TEXT-DECORATION:none;color:blue;' onclick='javascript:nextp("+index+");return false'>下一帧</a>&nbsp;&nbsp;<a href='#' style='TEXT-DECORATION:none;color:blue;' onclick='javascript:stopp("+index+");return false'>停  止</a>");
			$("#nextpage").html("&nbsp;&nbsp;&nbsp;<a href='#' style='TEXT-DECORATION:none;color:blue;' onclick='javascript:advanp("+index+");return false'>"+getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_syz")+"</a>&nbsp;&nbsp;<a id='next' href='#' style='TEXT-DECORATION:none;color:blue;' onclick='javascript:nextp("+index+");return false'>"+getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_xyz")+"</a>&nbsp;&nbsp;<a href='#' style='TEXT-DECORATION:none;color:blue;' onclick='javascript:stopp("+index+");return false'>"+getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_tz")+"</a>");
	...
	{
		//alert("无内容，无法预览！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_wnrwfyl"));
		return;
	}
}
//上一帧事件
...
function nextp(a){
	...
	{
		//alert("已经是最后一帧了");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_yjszhyzl"));
		return;
	}
	nextpage1 = 2;
}
...
function refresh()
{
	...
	else
	{
		...
		$("#nextpage").empty();
		$("#currentpage").empty();
		//$("#screen").html("<center style='padding-top:120px'><img src='"+commonPath+"/common/img/play.png' style='cursor:pointer;' title='播放' onclick='javascript:play();'/></center>");
		$("#screen").html("<center style='padding-top:120px'><img src='"+commonPath+"/common/img/play.png' style='cursor:pointer;' title='"+getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_bf")+"' onclick='javascript:play();'/></center>");
	}
   timer = null;
}
//播放方法
...
function reTimer()
{
	...
	if(first <= ttime)
	{
		...
		if(nextpage1 == 1){
			...
			if(index == 1){
				//alert("已经是第一帧了");
				alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_yjsdyzl"));
			}
			else{
				//第一帧播放的内容
...
function moveUp(obj){
	...
	if(curFrameId == 1){
		//alert("已经是第一帧，无法上移！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_yjsdyzwfsy"));
	}else{
...
function moveDown(obj){
	...
	if(frameSize == curFrameId){
		//alert("已经是最后一帧，无法下移！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_yjszhyzwfxy"));
	}else{
		//上移之前先保存当前选中帧
	    saveframe($(".mmsFrame").attr("id"));
