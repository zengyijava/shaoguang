...
$(document).ready(function() {
	...
	$("#mframes").children().click(function(){
		...
		$("#curf").html(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_dq")+$.trim($(this).attr("id"))+"/"+frameid);
	});
	$("#1").hover(function() {
	...
	$("#model").dialog({				
		autoOpen: false,
		//title:"添加图片",
		title:getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_tjtp"),
		height:200,
		width: 310
	});
	//添加声音的dialog
	$("#model2").dialog({				
		autoOpen: false,
		//title:"添加声音",
		title:getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_tjsy"),
		height:200,
		width: 310
	});
	//点击调用素材
	$("#addMat").click(function(){		
		...
		if(focusFrameId == "addFrame"){
			...
			//alert("请选择帧后再调用素材文件！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_qxzzhzdyscwj"));
			return ;
		}
		$("#mmsMat").dialog("open");
	});
	...
	$("#addImg").click(function(){		
		...
		if(focusFrameId == "addFrame"){
			//如果当前选中帧为“添加帧按钮”，则return
			//alert("请选择帧后再上传图片！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_qxxzhzsctp"));
			return ;
		}
		$("#model").dialog("open");
	});
	...
	$("#addMusic").click(function(){		
		...
		if(focusFrameId == "addFrame"){
			//如果当前选中帧为“添加帧按钮”，则return
			//alert("请选择帧后再上传声音文件！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_qxxzhzscsywj"));
			return ;
		}
		$("#model2").dialog("open");
	});
	   ...
	   var dsFlag = $("input[name:'templateType']:checked").val();
		if(dsFlag == 1 && paramCnt>0)
		{
    		...
    		for(var i=0;i<paramCnt;i++)
    		{
        		...
        		str=getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_cs")+(i+1)+': &nbsp;<input type="text" id="pa'+i+'" value=""/></br><br/>';
        		strs=strs+str;
    		}
...
function check(type){		
	...
	if (type == "chooseImg"){
		...
		{
			//alert("请选择要上传的图片文件！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_qxzyscdtpwj"));
			flag = false;
		}else{
		    ...
		    {
		        //alert("不支持的图片格式！");
		        alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_bzcdtpgs"));
		        flag =  false;
		    }
		}
		...
		{
			//alert("请选择要上传的声音文件！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_qxxyscdsywj"));
			flag = false;
		}
		else if (musics != "" && !(/^(mid|midi|amr)$/.test(musics.toString().substring((musics.toString().lastIndexOf("."))+1).toLowerCase())))
	    {
	        //alert("不支持的声音格式！");
	        alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_bzcdysgs"));
	        flag =  false;
	    }
		...
		if (tms == ""){
			//alert("请选择要上传的彩信文件！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_qxxyscdcxwj"));
			flag = false;
		}else {
			tms = tms.substring(tms.lastIndexOf(".")+1,tms.length);
	    	if(tms != "tms" && tms != "TMS"){
	    		//alert("不支持的彩信格式！");
	    		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_bzcdcxgs"));
		        flag =  false;
	    	}
	    }
	}
			...
			$.ajaxFileUpload ({ 
			    ...
			    success: function (data) { 
				    if(data != null && data.url == "false")
				    {
				    	...
				    	alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_scdwjdxbncg80kb"));
				    	return;
				    }
...
function getToatalSize(size,method)					
{
	...
	if (totalsize >parseFloat(80))
	{
		...
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_dqcxyjcgzdz80kbl"));
	}
...
function addFrame()    
{
    ...
    {
    	//alert("不能超过15帧！");
    	alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_bncg15z"));
    	return;
    }
    else
    {
    	...
    	$("#curf").html(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_dq")+$.trim(id)+"/"+$.trim((parseInt(frameid)+1)+""));
...
function delFrame(obj)				
{
	...
	if(id == 1){
		//alert("第一帧不允许删除");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_dyzbyxsc"));
		return;
    }
	//获取贞的数量
	var frameid = $("#framesize").attr("value");					
	if(frameid<1){
		//alert("没有可供删除的帧！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_mykscdz"));
		return;
	}
	//if(confirm("确定要删除当前帧吗？"))
	if(confirm(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_qdyscdqzm")))
	{
    	...
    	$.post(pathUrl+"/tem_mmsTemplate.htm?method=checkBadWord",{content:contents,lgcorpcode:lgcorpcode,templateType:templateType},function(result){
    		if (result != null && result == "")
    		{
    			$.post(pathUrl+"/tem_mmsTemplate.htm?method=update",{tmId:tmId,mt:mmsTitle,cont:contents,s:st,opType:opType,
    				...
    				{
    					if(opType == "add"){
    						//alert("模板新建成功！请等待运营商审核！");
    						alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_mbxjcgqddyyssh"));
        					//location.href = location;
    					}else if(opType == "edit"){
    						//alert("模板编辑成功！");
    						alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_mbbjcg"));
    					}else{
    						//alert("模板复制成功！");
    						alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_mbfzcg"));
    					}
    					location.href = pathUrl+"/tem_mmsTemplate.htm?lgguid="+lgguid;
    				}
    				else
...
{  
	...
	if (count != 0)			//是否帧  并且帧里面有图片 声音  内容
	{
		...
		else
		{
			...
			$("#nextpage").html("&nbsp;&nbsp;&nbsp;<a href='#' style='TEXT-DECORATION:none;color:blue;' onclick='javascript:advanp("+index+");return false'>"+getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_syz")+"</a>&nbsp;&nbsp;<a id='next' href='#' style='TEXT-DECORATION:none;color:blue;' onclick='javascript:nextp("+index+");return false'>"+getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_xyz")+"</a>&nbsp;&nbsp;<a href='#' style='TEXT-DECORATION:none;color:blue;' onclick='javascript:stopp("+index+");return false'>"+getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_tz")+"</a>");
	...
	{
		//alert("无内容，无法预览！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_wnrwfyl"));
		return;
	}
}
//上一帧事件
...
function nextp(a){
	...
	{
		//alert("已经是最后一帧了");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_yjszhyzl"));
		return;
	}
	nextpage1 = 2;
}
...
function refresh()
{
	...
	else
	{
		...
		$("#screen").html("<center style='padding-top:120px'><img src='"+commonPath+"/common/img/play.png' style='cursor:pointer;' title='"+getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_bf")+"' onclick='javascript:play();'/></center>");
	}
   timer = null;
}
//播放方法
...
function reTimer()
{
	...
	if(first <= ttime)
	{
		...
		if(nextpage1 == 1){
			...
			if(index == 1){
				//alert("已经是第一帧了");
				alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_yjsdyzl"));
			}
			else{
				//第一帧播放的内容
...
function moveUp(obj){
	...
	if(curFrameId == 1){
		//alert("已经是第一帧，无法上移！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_yjsdyzwfsy"));
	}else{
...
function moveDown(obj){
	...
	if(frameSize == curFrameId){
		//alert("已经是最后一帧，无法下移！");
		alert(getJsLocaleMessage("ydcx","ydcx_cxyy_dtcxfs_yjszhyzwfxy"));
	}else{
		//上移之前先保存当前选中帧
	    saveframe($(".mmsFrame").attr("id"));
