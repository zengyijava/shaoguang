...
$(document).ready(function() {
	...
	if(pathtype == 2){
		$("#tempType").removeAttr("checked");
		$("#tempType1").attr("checked", "checked");
	}
	selectType();
});
...
function doUp(type)	{
	...
	if (check(type)){		//这里判断上传的 图片或者声音是否合法	
		if(type == "chooseImg"){
			$.ajaxFileUpload ({ 
			    url:pathUrl+'/tem_mmsTemplate.htm?method=upload&lgcorpcode='+lgcorpcode+'&lguserid='+lguserid, //处理上传文件的服务端 
...
function doSubmit()
{
    ...
    else
	{
    	...
    	$.post(pathUrl+"/tem_mmsTemplate.htm?method=checkBadWord",{content:contents,lgcorpcode:lgcorpcode,templateType:templateType},function(result){
    		if (result != null && result == "")
    		{
    			$.post(pathUrl+"/tem_mmsTemplate.htm?method=update",{tmId:tmId,mt:mmsTitle,mmsCode:mmsCode,hiddenCode:hiddenCode,cont:contents,s:st,opType:opType,
