	...
	if(confirm(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_nqdyscgtxxm")))
	{
		$.post("tem_mmsTemplate.htm?method=delete",{ids:i,lgcorpcode:$("#lgcorpcode").val()},function(result){
			if(result>0)
			{
				...
				var url = path+'/tem_mmsTemplate.htm';
				var conditionUrl = "";
				if(url.indexOf("?")>-1)
				{
					conditionUrl="&";
...
function doPreview(msg,dsFlag)
{
	...
	$.post(pathUrl+"/tem_mmsTemplate.htm?method=getTmMsg",{tmUrl:msg},function(result){
...
function chState(id,t)
{
    ...
    {
    	 var pathUrl = $("#pathUrl").val();
        $.post(pathUrl+"/tem_mmsTemplate.htm?method=changeState",{id:id,t:t},function(result){
...
function doExport(u)
{
	...
	$.post(pathUrl+"/tem_mmsTemplate.htm?method=checkMmsFile", {
		url : u
			...
			if (result == "true") {
				download_href(pathUrl+"/tem_mmsTemplate.htm?method=exportTms&u="+u);
	...
	$.post(pathUrl+"/tem_mmsTemplate.htm?method=checkMmsFile", {
		...
		function(result) {
			if (result == "true") {
				...
				location.href=pathUrl+"/tem_mmsTemplate.htm?method=doCopy&tmId="+tmId+"&opType="+opType+"&lgcorpcode="+lgcorpcode;
			} else if (result == "false"){
			    if(opType == "copy"){
			    	//alert("文件不存在，无法复制！");
...
function doUp(){
		...
		if (importCode == "") {
			...
			return;
		}else if(escape(importCode).indexOf("%u")!=-1){
			//alert("模板编号不能包含汉字！");
			alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbbhbnbhhz"));
			return;   
		}else if(pattern.test(importCode)){
			//alert("模板编号不能包含特殊字符！");
   ...
   var data = {lguserid:lguserid,lgcorpcode:lgcorpcode};
	   $.ajaxFileUpload ({ 
	    url:pathUrl+'/tem_mmsTemplate.htm?method=uploadTms&lgcorpcode='+lgcorpcode+'&mmsCode='+importCode, //处理上传文件的服务端 
	    ...
	    success: function (result) { 
	        if(result == "typeNotMatch"){
	        	...
	        	alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_drmbztphypgsbzc"));
	        }else if(result == "repeat"){
	        	//alert("模板编号重复！");
	        	alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_mbbhcf"));
	        }else if(result == "true"){
	        	//alert("导入模板成功！");
	        	alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_drmbcg"));
	        	location.href = pathUrl+"/tem_mmsTemplate.htm?method=find&lgguid="+$("#lgguid").val();
	        }else{
	        	//alert("导入模板失败！");
	        	alert(getJsLocaleMessage("ydcx","ydcx_cxyy_mbbj_drmbsb"));
	        }
	    }
	});
}
...
function doAdd()
{
 //1静态彩信  2动态彩信  3新增
	 var pathUrl = $("#pathUrl").val();
   window.location.href=pathUrl+'/tem_mmsTemplate.htm?method=doAdd&type=3&lgcorpcode='+$("#lgcorpcode").val();
}
...
function showShareTmp(tmId, tmName,userId) {
	...
	var lgcorpcode = $("#lgcorpcode").val();
	var iPath = $("#iPath").val();
	var ScommonPath = $("#pathUrl").val();
	tmName=tmName||'-';
//	tmName=encodeURI(tmName);
//	tmName=encodeURI(tmName);
	//选择类型查询  1.员工通讯录   2.客户通讯录  3.群组(员工+客户) 4.员工群组 5.客户群组 6.客户属性 7.签约用户  8.操作员机构
	var chooseType = "8";
	if(CstlyeSkin.indexOf("frame4.0") != -1) {
        tmName = encodeURI(encodeURI(tmName));
		$("#flowFrame").attr("src",ScommonPath+"/common/selectUserInfo.jsp?lguserid="+lguserid+"&chooseType="+chooseType+"&lgcorpcode="+lgcorpcode+"&tmId="+tmId+"&tmName="+tmName);
	...
	resizeDialog($("#shareTmpDiv"),"ydwxDialogJson","tem_shareTemplate_shareTmpDiv");
...
function updateShareTemp() {
	...
	$.post(pathUrl + "/tem_mmsTemplate.htm", {
