...
function setDefault()
{
	if(confirm( getJsLocaleMessage("ydcx","ydcx_cxyy_jtcxfs_text_4_1"))) {
		var lguserid = $('#lguserid').val();
		var lgcorpcode = $('#lgcorpcode').val();
		var mmsUser = $("#mmsUser").val();
		$.post("smm_sameMms.htm?method=setDefault", {
			lguserid: lguserid,
			...
			if (result == "seccuss") {
				alert( getJsLocaleMessage("ydcx","ydcx_cxyy_jtcxfs_text_4_2"));
				return;
			} 
			else if(result == "fail"){
				alert( getJsLocaleMessage("ydcx","ydcx_cxyy_jtcxfs_text_5"));
				return;
			}
		});
	}
