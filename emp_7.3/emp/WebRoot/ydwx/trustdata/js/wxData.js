$(document).ready(function() {
	...
	var Dwidth  = 450;//非frame4.0默认宽度
	var Dheight = 280;//非frame4.0默认高度
	...
	$("#addTypeDiv").dialog({
		...
		close:function(){
            ...
            $.post(path + "/wx_ueditor.htm?method=getActiviTables",{lgcorpcode:lgcorpcode},function(data){
                    ...
                    $("#newSelect_01").remove();
                    tname.isSearchSelectNew({'id' : 'newSelect_01', 'width': '302', 'enableInput': false, 'zIndex': 0});
           });
        }
	});
...
function manageType()
{
	...
	$("#tempFrame").addClass("ydwx_typeDailog");
...
function doreturn(){
	...
	window.location.href = path+"/wx_trustdata.htm?method=find&skip=1";
}
...
function submitFormAddhdx(){
	...
	var lguserid = $("#lguserid").val();
	var lgcorpcode = $("#lgcorpcode").val();
	var replySetType = $('input[name="replySetType"]:checked').val();
