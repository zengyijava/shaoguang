...
$(function() {
	$.fn.smohanfacebox = function(options) {
		...
		$("#"+options.divid).prepend("<div id='faceBox'><span class='Corner'></span><div class='faceInner'><h3><span>常用表情</span><a class='close' title='关闭'></a></h3><ul>"+faceimg+"</ul></div></div>");
	     $('#faceBox').css("display",'none');//创建完成后先将其隐藏
		//创建表情框结束
		
		var $facepic = $("#faceBox li img");
		//BTN触发事件，显示或隐藏表情层
		$btn.click(options.Event,function(e) {
		...
		$facepic.click(function(){
			 ...
			 $("#"+options.textid).insertContent($(this).attr("face"));
			 $btn.removeClass('in');
			});			
		//关闭表情层
