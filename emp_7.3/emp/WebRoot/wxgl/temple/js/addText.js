	$(document).ready(function() {
	   ...
	   var textare = getJsLocaleMessage("wxgl","wxgl_qywx_nrhfgl_text_33");
	   var path=$("#path").val();
	  function back()
	  {
		  history.go(-1);
	 ...
	 function save(o){
		...
		if(isnullstr(replyname)){
		 ...
		 var words = "";
            ...
            $.post(path+"/weix_keywordReply.htm?method=saveTextReply",
					{
            			words:words,
            			replyname:replyname,
            			accuntnum:accuntnum,
					...
					function(r){
		            		 if(r!=null&&r=="success")
		                     {
		                     	alert(getJsLocaleMessage("wxgl","wxgl_qywx_nrhfgl_text_42"));
		                     	location.href = path+'/weix_keywordReply.htm?lgcorpcode='+$("#lgcorpcode").val()+'&pageSize='+$("#pageSize").val();
		                     }
		                     else if(r!=null && r=="fail")
		                     {
		                       alert(getJsLocaleMessage("wxgl","wxgl_qywx_nrhfgl_text_43"));
		                     }
		                     else
		                     {
		                         alert(getJsLocaleMessage("wxgl","wxgl_qywx_nrhfgl_text_44"))
		                     }
		               });
