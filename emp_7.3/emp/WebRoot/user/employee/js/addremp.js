...
function delBk()
{
	...
	if(confirm(getJsLocaleMessage('employee','employee_alert_7'))){
	    ...
	    $.ajax({
	        ...
	        type: 'post',
	        error: function(){
	           alert(getJsLocaleMessage('employee','employee_alert_8'));		
	           return;
	        },
	        success: function(result){
				...
				if (result>0) {
					alert(getJsLocaleMessage('employee','employee_alert_9')+result+getJsLocaleMessage('employee','employee_alert_10'));
					window.location.href=location;
				}else if(result<=0){
...
function showPageInfo(){
	...
	$pa.append('<span id="p_info">'+getJsLocaleMessage('employee','employee_alert_12')+totalRec+getJsLocaleMessage('employee','employee_alert_13')+pageIndex+'/'+total+getJsLocaleMessage('employee','employee_alert_15')+pageSize+getJsLocaleMessage('employee','employee_alert_14')+'</span>');
	...
	newDiv.innerHTML = "<center><div>"+getJsLocaleMessage('employee','employee_alert_33')+"<input name='page_input' id='page_input' onfocus='jumpDisplay(1)' onblur='jumpDisplay(0)' style='height: 15px;' id='page_input' type='text' value='"+pageIndex+"' size='4' />"+getJsLocaleMessage('employee','employee_alert_15') +
		"<input name='size_input' id='size_input' onfocus='jumpDisplay(1)' onblur='jumpDisplay(0)' style='height: 15px;' id='size_input' type='text' value='"+pageSize+"' size='4' />"+getJsLocaleMessage('employee','employee_alert_14')+"</div>" +
...
function jumpSure()
{
	...
	if(size<1 ||  !checkPage.test(size) ){
		alert(getJsLocaleMessage('employee','employee_alert_18'));
		$("#size_input").focus();
		return;
	}
	if(size>500)
...
$(function(){
	$('#addToGroup').click(function (){
		...
		if(i==0)
		{
			alert(getJsLocaleMessage('employee','employee_alert_21'));
		}else
		{
...
function addL2G()
{
	...
	{
		alert(getJsLocaleMessage('employee','employee_alert_22'));
	}else
	{
		$.post("updateGroup.htm",{
			...
			if(count>=0)
			{
				alert(getJsLocaleMessage('employee','employee_alert_23')+count+getJsLocaleMessage('employee','employee_alert_24')
	...
	case 1:
		$.post("a_employeeAddrBook.htm?method=sync",{opType:index},
			function (result)
			{
				if(result>-1)
				{
					alert(getJsLocaleMessage('employee','employee_alert_25'));
					window.location.reload();
				}else
				{
					alert(getJsLocaleMessage('employee','employee_alert_26'));
				}
			}
		);
		break;
	case 2:
		$.post("a_employeeAddrBook.htm?method=sync",{opType:index},
				function (result)
				{
					if(result>-1)
					{
						alert(getJsLocaleMessage('employee','employee_alert_27'));
						window.location.reload();
					}else
					{
						alert(getJsLocaleMessage('employee','employee_alert_28'));
					}
				}
			);
		break;
	default :
		$.post("a_employeeAddrBook.htm?method=sync",{opType:index},
				function (result)
				{
					if(result>-1)
					{
						alert(getJsLocaleMessage('employee','employee_alert_29'));
						window.location.reload();
					}else
					{
						alert(getJsLocaleMessage('employee','employee_alert_30'));
					}
				}
			);
	}
...
function toSysuser()
{
	...
	if(count != "1"){alert(getJsLocaleMessage('employee','employee_alert_31'));return;}
	else
	{
		var isopr = $("input[name='delBook']:checked").attr("id");
		if (isopr == "1")
		{
			alert(getJsLocaleMessage('employee','employee_alert_32'));
			return;
		}
