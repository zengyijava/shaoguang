	    ...
	    $.ajax({
	        ...
	        success: function(result){
	        	if(result.indexOf("html") > 0){
	        		...
	        		//alert(pathUrl + "/epl_employeeBook.htm?method=find&lgusername="+lgusername+"&lgguid="+lgguid+"&lguserid="+lguserid+"&lgcorpcode="+lgcorpcode);
	        		//window.location.href = pathUrl + "/epl_employeeBook.htm?method=find&lgusername="+lgusername+"&lgguid="+lgguid+"&lguserid="+lguserid+"&lgcorpcode="+lgcorpcode;
	        		submitForm();
	    		    return;
	    		}
				if (result>0) {
                    /*删除选中记录成功,共删除  条记录！ */
                    alert(getJsLocaleMessage("common","common_js_addrBook_4")+result+getJsLocaleMessage("common","common_js_addrBook_5"));					//window.location.href=location;
					//alert(pathUrl + "/epl_employeeBook.htm?method=find&lgusername="+lgusername+"&lgguid="+lgguid+"&lguserid="+lguserid+"&lgcorpcode="+lgcorpcode);
					//window.location.href = pathUrl + "/epl_employeeBook.htm?method=find&lgusername="+lgusername+"&lgguid="+lgguid+"&lguserid="+lguserid+"&lgcorpcode="+lgcorpcode;
					submitForm();
				}else if(result<=0){
...
function submitForm()
{
	...
	var submitSartTime=$('#submitSartTime').val();
...
/*使用get方式提交     新增p  参数如果为中文需要进行编码，还未处理，进一步排查
function submitForm()
{
	page_loading();
	var phone=$('#phone').val();
	...
	var userName = $("#userName").val();
		...
		$('#bookInfo').load(url+"&dataType=table&depId="+depId+"&lgcorpcode="+lgcorpcode+"&lguserid="+lguserid+"&mobile="+phone+"&name="+name+"&pageIndex="+pageIndex+"&pageSize="+pageSize+"&lgguid="+lgguid+"&submitEndTime="+submitEndTime+"&submitSartTime="+submitSartTime+"&option="+option+"&sexy="+sexy+"&userName="+userName+"&isDes="+isDes,function(){
			afterSubmitForm();
		});
