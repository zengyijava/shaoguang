 ...
 * @datetime 2013-11-22 上午09:49:11
 * @description
 */
public class SerMtTaskSql {
	...
	public static String getFieldSql()
	{
		...
		String sql = new StringBuffer("select mttask.").append(
				...
				TableLfMttask.TASKID).append(",").append(
				TableLfService.SER_ID).append(",").append(
				TableLfMttask.TITLE).append(" SER_NAME ")
				.toString();
		return sql;
	}
	/**
	...
	public static String getmttaskTableSqlTwo(String tableName,LinkedHashMap<String, String> conditionMap)
	{
		   ...
		   .append(",ml.").append(TableMtTask.MESSAGE).append(",ml.").append(TableMtTask.ERROR_CODE)
		   .append(",ml.").append(TableMtTask.PK_NUMBER).append(",ml.").append(TableMtTask.PK_TOTAL)
		   .append(" from ").append(TableMtTask.TABLE_NAME).append(" ml ").append(StaticValue.getWITHNOLOCK());
...
//				.append(TableLfMttask.TABLE_NAME).append(" mttask ").append(" inner join lf_sysuser sysuser on mttask.user_id=sysuser.user_id").toString();
		String sql = new StringBuffer(" from ")
		.append(TableLfMttask.TABLE_NAME).append(" mttask ").append(StaticValue.getWITHNOLOCK()).append(" left join LF_SERVICE service on mttask.TEMP_ID=service.SER_ID where mttask.CORP_CODE= '").append(corpCode).append("' ").toString();
		return sql;
	}
	/**
	 * 组装查询语句
...
//				.append(dominationSql).append("))").toString();
	...
	public static String getConditionSql(LfMttaskVo lfMttaskVo)
	{
		...
		{
			conditionSql.append(" and mttask.")
					.append(TableLfMttask.SEND_STATE).append(" = ").append(
							lfMttaskVo.getSendstate());
		}
		...
		if (lfMttaskVo.getSerId() != null)
		{
			conditionSql.append(" and service.")
					.append(TableLfService.SER_ID).append(" =").append(
							lfMttaskVo.getSerId());
		}
		
		if (lfMttaskVo.getSerName() != null
				&& !"".equals(lfMttaskVo.getSerName()))
		{
			conditionSql.append(" and mttask.")
					.append(TableLfMttask.TITLE).append(" like '%").append(
							lfMttaskVo.getSerName()).append("%' ");
		}
