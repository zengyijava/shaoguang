...
import com.montnets.emp.tailmanage.dao.TailMgrDAO;
import com.montnets.emp.util.PageInfo;


public class TailMgrBiz extends SuperBiz{
	
	
	TailMgrDAO dao=new TailMgrDAO();
		  ...
		  String ret="false";
			...
			try {
				empTransactionDAO.beginTransaction(conn);
					...
					conditionMap.put("smstail_id", tailid);
					 empTransactionDAO.delete(conn, LfBusTail.class, tailid);
					 //删除关联关系表
					 size=empTransactionDAO.delete(conn, LfBusTailTmp.class, conditionMap);
	 ...
	 */
		  ...
		  String ret="false";
			...
			try {
				...
				for(int i=0;i<idarr.length;i++){
					...
					LinkedHashMap<String, String> conditionMap = new LinkedHashMap<String, String>();
					conditionMap.put("bustail_id", idarr[i]);
					 empTransactionDAO.delete(conn, LfBusTail.class, conditionMap);
					 LinkedHashMap<String, String> condition = new LinkedHashMap<String, String>();
					 condition.put("smstail_id", idarr[i]);
					 //删除关联关系表
					 size=empTransactionDAO.delete(conn, LfBusTailTmp.class, condition);
				}
	   ...
	   * @return
	   */
	public boolean save(LfBusTail tail,String bussids){
		boolean add=false;
		int  dep_id=0;
		try {
			LfSysuser user=new BaseBiz().getLfSysuserByUserId(tail.getUser_id()+"") ;
			if(user!=null){
		...
		try {
			...
			for(int i=0;i<ids.length;i++){
				if(!"".equals(ids[i])){
					LfBusTailTmp tmp =new LfBusTailTmp();
					tmp.setAssociate_type(0);
					tmp.setCorp_code(tail.getCorp_code());
					tmp.setCreate_time(new Timestamp(System.currentTimeMillis()));
					tmp.setDep_id(dep_id);
					tmp.setBus_id(Integer.parseInt(ids[i]));
					tmp.setSmstail_id(tailid.intValue());
					tmp.setUser_id(tail.getUser_id());
					add=empTransactionDAO.save(conn, tmp);
				}
			}
			empTransactionDAO.commitTransaction(conn);
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public boolean update(LfBusTail tail,String bussids,String modfid){
		boolean add=false;
		int  dep_id=0;
		try {
			LfSysuser user=new BaseBiz().getLfSysuserByUserId(tail.getUser_id()+"") ;
		...
		try {
			...
			empTransactionDAO.delete(conn, LfBusTailTmp.class, conditionMap);
			...
			for(int i=0;i<ids.length;i++){
				if(!"".equals(ids[i])){
					...
					tmp.setDep_id(dep_id);
					tmp.setBus_id(Integer.parseInt(ids[i]));
					tmp.setSmstail_id(Integer.parseInt(modfid));
					tmp.setUser_id(tail.getUser_id());
					tmp.setUpdate_time(new Timestamp(System.currentTimeMillis()));
					add=empTransactionDAO.save(conn, tmp);
				}
			}
			empTransactionDAO.commitTransaction(conn);
