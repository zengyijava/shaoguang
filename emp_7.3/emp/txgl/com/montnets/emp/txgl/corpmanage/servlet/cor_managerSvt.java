package com.montnets.emp.txgl.corpmanage.servlet;
...
import com.montnets.emp.txgl.corpmanage.biz.CorManagerBiz;
import com.montnets.emp.txgl.corpmanage.biz.EnterpriseManageBiz;
...
public class cor_managerSvt extends BaseServlet {
	...
	final EnterpriseManageBiz enterpriseManageBiz = new EnterpriseManageBiz();

	final CorManagerBiz corManagerBiz = new CorManagerBiz();
	...
	public void find(HttpServletRequest request, HttpServletResponse response)
		...
		try
		{
			...
			//跳转页面
			request.getRequestDispatcher(this.empRoot+this.basePath + "/cor_manager.jsp")
	...
	public void add(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			//跳转页面
			request.getRequestDispatcher(this.empRoot+this.basePath + "/cor_editCorp.jsp")
					.forward(request, response);
		} catch (Exception e) {
		...
		try
		{
			...
			if(lgCorpCode != null && "100000".equals(lgCorpCode))
			{
					...
					try {
						...
						if (userList.size() > 0) {
							...
							request.getRequestDispatcher(
									this.empRoot + this.basePath + "/cor_corpAdmin.jsp").forward(
											request, response);
						} else
							action = "add";
					} catch (Exception e) {
				...
				if ("add".equals(action)) {
					...
					request.setAttribute("user", user);
					request.getRequestDispatcher(this.empRoot + this.basePath + "/cor_addAdmin.jsp")
					.forward(request, response);
				}
			}
			else
...
//						boolean updateRs = baseBiz.updateObj(user);
	...
	public void upload(HttpServletRequest request, HttpServletResponse response) throws IOException
	{
		try {
			...
			{
				//路径
				String dirUrl =new TxtFileUtil().getWebRoot()+this.empRoot+ "/" + SystemGlobals.getValue(StaticValue.EMP_WEB_FRAME) + StaticValue.LOGOPATH;
				String imgUrl = "";
	...
	public void toParamSet(HttpServletRequest request, HttpServletResponse response)
		...
		try
		{
			...
			//跳转页面
			request.getRequestDispatcher(this.empRoot+this.basePath + "/cor_ParamSet.jsp")
			.forward(request, response);
		}
		catch (Exception e)
		{
...
/*	public void setProxyAccount(HttpServletRequest request, HttpServletResponse response)
    ...
    public void rmsAuthManage(HttpServletRequest request, HttpServletResponse response) {
        try {
        	String pageIndex = "1";
			String corpCode = corManagerBiz.getCorpCode();
            request.setAttribute("corpCode", corpCode);
            Map<String,String> map=new HashMap<String,String>();
			//获取已存在表的数据
            ...
            request.getRequestDispatcher(this.empRoot+this.basePath + "/cor_authConfig.jsp")
                    .forward(request, response);
