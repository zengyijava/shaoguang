...
public class SuperTxglDAO 
{
	...
	public boolean executeBySQL(String sql, String POOLNAME) throws Exception
	{
		...
		{
			EmpExecutionContext.error(errorLoger.getErrorLog(e,"DAO方法"+Thread.currentThread().getStackTrace()[1].getMethodName()+"执行失败！"));
			throw e;
		} finally
		{
			try
			...
			{
				EmpExecutionContext.error(errorLoger.getErrorLog(e,"关闭数据库资源出错！"));
			}
		}
		return b;
	}
			...
			Map<String, String> columns) throws Exception
	{
		...
		} finally
		{
			...
			{
				EmpExecutionContext.error(errorLoger.getErrorLog(e, "根据sql获取对象集合异常。"));
			}
		}
		return returnList;
	}
			...
			throws Exception
	{
		...
		} finally
		{
			...
			{
				EmpExecutionContext.error(errorLoger.getErrorLog(e, "根据sql语句查询并获取对象集合异常。"));
			}
		}
		return returnList;
	}
			...
			Map<String, String> columns)
	{
		...
		{
			EmpExecutionContext.error(errorLoger.getErrorLog(e, "结果集处理异常！"));
	...
	protected Long getSequenceNextValue(Connection conn, String sequenceName)
	{
		...
		{
			EmpExecutionContext.error(errorLoger.getErrorLog(e, "获取序列下一个id异常。"));
		} finally
		{
			try
			{
	...
	protected int findCountBySQL(String countSql)
	{
		...
		{
			EmpExecutionContext.error(errorLoger.getErrorLog(e, "根据sql语句查询数量异常。"));
		} finally
		{
			try
			{
			...
			throws Exception
	{
		...
		{
			EmpExecutionContext.error(errorLoger.getErrorLog(e,"查询总页数出错！"));
		} finally
		{
			try
			{
	...
	public long getIdByPro(int fildId, int count)
	{
		...
		{
			EmpExecutionContext.error(errorLoger.getErrorLog(e, "通过存储过程获取自增值id异常。"));
			return 0;
		} finally
		{
			try
			...
			{
				EmpExecutionContext.error(errorLoger.getErrorLog(e, "关闭数据库资源异常。"));
			}
		}
	}
