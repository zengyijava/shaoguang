...
public class PageFieldConfigBiz 
{
	...
	public boolean getPageFieldList(){
		try{
			...
			EmpExecutionContext.error(errorLoger.getErrorLog(e, "获取显示的控件和控件下的子项异常。"));
			return false;
		}
	}
	...
	public List<LfPageField> getPageFieldById(String fieldId){
		try{
			...
			EmpExecutionContext.error(errorLoger.getErrorLog(e, "根据编号获取控件集合异常。"));
			return null;
		}
	}
	...
	private boolean setSubFieldMap(LfPageField pageField){
		...
		try{
			...
			EmpExecutionContext.error(errorLoger.getErrorLog(e, "设置控件子项异常。"));
			return false;
		}
	}
}
