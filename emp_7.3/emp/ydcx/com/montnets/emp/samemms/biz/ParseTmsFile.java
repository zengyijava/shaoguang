	...
	public List<TmsSmilItem> Parse(String filePath)
    {
        ...
        try
        {
            if( !new File(filePath).exists() )
            {
            	...
            	EmpExecutionContext.error("TMS文件不存在！");
            	return null;
            }
            ...
            if( bytes[0] != (byte)161 || bytes[1] != (byte)100 )
            {
            	...
            	EmpExecutionContext.error("文件不是可以识别的版本格式！");
            	return null;
            }
...
/*            int totalLength = (int)(bytes[index + 3] | bytes[index + 2] << 8 | bytes[index + 1] << 16 | bytes[index] << 24);
            ...
            if( fileCount <= 0 )
            {
                ...
                EmpExecutionContext.error("tms包含的文件数量不能少于等于0");
                return null;
            }
...
/*                if(i == 0){
            ...
            if( count != 1 )
            {
                //throw new Exception("tms文件中必须包含一个smil文件，并且只能包含一个smil文件！");
            	 EmpExecutionContext.error("tms文件中必须包含一个smil文件，并且只能包含一个smil文件！");
            	 return null;
            }
        ...
        {
        	EmpExecutionContext.error(e,"处理tms文件出现异常！");
        	return smilItemsList;   
        }finally{
        	if(in!=null){
        		try {
					...
					EmpExecutionContext.error(e,"处理tms文件出现异常！关闭文件流");
				}
        	}
        }
 ...
 /*   private void CheckFiles(TmsFileItem smilFile)
	...
	public static void main(String[] args) throws Exception
	{
		try{
			...
			EmpExecutionContext.error(e,"测试彩信模板出现异常！");
		}
