...
import com.montnets.emp.wxgl.biz.ReplyBiz;
 ...
 * @author chensj
 */
public class weix_keywordReplySvt extends BaseServlet {
    private final String PATH = "/wxgl/temple";
    ...
    public void find(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            List<LfWeiAccount> accoutlist = baseBiz.getByCondition(LfWeiAccount.class, conditionMap, null);
            if (!isFirstEnter) {
                // 模板类型
            ...
            if (loginSysuserObj != null) {
                ...
                EmpExecutionContext.info("关键字回复", loginSysuser.getCorpCode().toString(), loginSysuser.getUserId().toString(),
                        loginSysuser.getUserName(), opContent, "GET");
            }

            request.getRequestDispatcher(PATH + "/weix_tmpList.jsp").forward(request, response);
        }
    }
    ...
    public void newText(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            request.getRequestDispatcher(PATH + "/weix_addTextReply.jsp").forward(request, response);
        }
    }
    ...
    public void doEditemp(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String keywords = "";
        // 跳转页面
        String page = "/weix_ediTextReply.jsp";
        List<LfWeiAccount> accoutlist = new ArrayList<LfWeiAccount>();
        LfWeiTemplate te = new LfWeiTemplate();
        if (GlobalMethods.isInvalidString(msgtype)) {
            msgtype = "0";
        }
        if ("2".equals(msgtype)) {
            page = "/weix_editMutiImgTmp.jsp";
        }
        if ("1".equals(msgtype)) {
            page = "/weix_editSingleImgTmp.jsp";
        }
        try {
            // 回复名称
            ...
            LinkedHashMap<String, String> conditionMap = new LinkedHashMap<String, String>();
            conditionMap.put("corpCode", lgcorpcode);
            accoutlist = baseBiz.getByCondition(LfWeiAccount.class, conditionMap, null);
            // 模板的关键字
            keywords = tDao.getkeyname(tid);
            // 模板
            te = baseBiz.getById(LfWeiTemplate.class, tid);
            ...
            if ("0".equals(msgtype)) {
                ...
                // 图文
                List<LfWeiRimg> echorimgs = TempleDao.getLfWcRimgbyids(te.getRimgids());
                String time = "";
    ...
    public void preview(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            ...
            if (!GlobalMethods.isInvalidString(tetId)) {
                LfWeiRtext rtext = baseBiz.getById(LfWeiRtext.class, tetId);
                if (rtext != null) {
                ...
                LfWeiRevent revent = baseBiz.getById(LfWeiRevent.class, evtId);
                if (revent != null) {
            ...
            if (hasTemplate && !GlobalMethods.isInvalidString(tempId)) {
                // 回复管理中的模板ID
                LfWeiTemplate template = baseBiz.getById(LfWeiTemplate.class, tempId);
                ...
                // 取出摘要信息
                request.setAttribute("summary", template.getMsgText());
            }
            ...
            request.getRequestDispatcher(PATH + "/weix_tmpPreview.jsp").forward(request, response);
        } catch (Exception e) {
            EmpExecutionContext.error(e, "回复管理-回复预览失败!");
            request.getRequestDispatcher(PATH + "/weix_tmpPreview.jsp").forward(request, response);
        }
    }
    ...
    public void uploadImg(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            message = "oversize";
            EmpExecutionContext.error(e, "回复管理-图片上传超过尺寸！");
        } catch (Exception e) {
            message = "error";
            EmpExecutionContext.error(e, "回复管理-图片上传失败！");
        } finally {
    ...
    public void saveOrUpdateImgReply(HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        List<LfWeiRimg> vorimgs = new ArrayList<LfWeiRimg>();
        LfWeiRimg rimg;
        if (!GlobalMethods.isInvalidString(item_ids)) {
            ...
            for (String s : items) {
                ...
                summary = request.getParameter("item-summary" + s);
                content = request.getParameter("item-content" + s);
                // 处理特殊的空格，此问题出现在复制网页内容时，保存数据库变成问号
                content = content.replace(" ", "&nbsp;");
                link = request.getParameter("item-link" + s);
                source_url = request.getParameter("item-source" + s);
                rimg.setTitle(title);
                rimg.setPicurl(url);
                rimg.setDescription(content);
                rimg.setLink(link);
                rimg.setSummary(summary);
                rimg.setSourceUrl(source_url);
                rimg.setAId(aid);
                vorimgs.add(rimg);
            }
        }
        ...
        String msg = "";
        if (flag) {
            msg = MessageUtils.extractMessage("wxgl", "wxgl_java_title_17", request);
        } else {
            msg = MessageUtils.extractMessage("wxgl", "wxgl_java_title_18", request);
        }
        if (GlobalMethods.isInvalidString(tid)) {
            msg = msg.replace("{OPP}", MessageUtils.extractMessage("wxgl", "wxgl_java_title_19", request));
        } else {
            msg = msg.replace("{OPP}", MessageUtils.extractMessage("wxgl", "wxgl_java_title_20", request));
        }
    ...
    public void addImgTmp(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (type != null && "1".equals(type)) {
            page = "weix_addMutilImgTmp.jsp";
        } else {
            page = "weix_addSingleImgTmp.jsp";
        }
        try {
            BaseBiz baseBiz = new BaseBiz();
            LinkedHashMap<String, String> conditionMap = new LinkedHashMap<String, String>();
            ...
            EmpExecutionContext.error(ex, "回复管理-添加图文页面加载失败！");
        } finally {
            request.getRequestDispatcher(PATH + "/" + page).forward(request, response);
        }
    }
