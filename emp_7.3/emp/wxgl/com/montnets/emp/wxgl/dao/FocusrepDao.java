...
import com.montnets.emp.table.wxgl.TableLfWeiRevent;
import com.montnets.emp.util.PageInfo;

public class FocusrepDao extends SuperDAO
{
	/**
	 * 默认回复查询(分页和不分页两种情况)
	 * 
	...
	public List<DynaBean> findFocusReply(LinkedHashMap<String, String> conditionMap, PageInfo pageInfo) throws Exception
	{
		...
		String tableSql = " from "+TableLfWeiRevent.TABLE_NAME+" revent " + StaticValue.getWITHNOLOCK() + " left join "+TableLfWeiAccount.TABLE_NAME + " account" + StaticValue.getWITHNOLOCK() + " on revent.a_id=account.a_id ";
		// 查询条件
		StringBuffer conSql = new StringBuffer();
		conSql.append(" where  ");
		conSql.append("  revent.evt_type = 1");
	...
	public List<LfWeiAccount> findBindAccountByCorpCode(String corpCode, String status) throws Exception
	{
		...
		tableSql.append(" FROM ").append(TableLfWeiAccount.TABLE_NAME).append(" account ").append(StaticValue.getWITHNOLOCK()).append(" where  ");

		StringBuffer conSql = new StringBuffer();
		conSql.append("  account.").append(TableLfWeiAccount.CORP_CODE).append("='").append(corpCode).append("'");
		if("0".equals(status))
		{
		...
		conSql.append("select revent.A_ID from " + TableLfWeiRevent.TABLE_NAME + " revent "+StaticValue.getWITHNOLOCK()+" where revent.CORP_CODE = " + "'" + corpCode + "'");
		conSql.append(")");
		conSql.append(")");
		conSql.append(" order by CREATETIME DESC");

		String sql = filedSql + tableSql.toString() + conSql.toString();
		acctList = findEntityListBySQL(LfWeiAccount.class, sql, StaticValue.EMP_POOLNAME);
		return acctList;
	}
}
