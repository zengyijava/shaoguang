 ...
 *
 */
public class ReviewRemindMOStart extends SuperBiz implements IBusinessStart{
	/**
	 * 上行启动该方法
	 */
	@Override
    public boolean start(LfMotask moTask) throws Exception {
		...
		String resultStr = judgeMotask(moTask);
			try{
				...
				if("0".equals(isre) && "success".equals(resultStr))
				{
					...
					if(msg != null && !"".equals(msg))
					{
						...
						}else
						{
							...
							if("success".equals(resultStr) && examineSms!=null){
								//获取该条短信提醒记录
								LfFlowRecord flowRecord = baseBize.getById(LfFlowRecord.class,examineSms.getFrId());
								if(flowRecord == null){
										...
										if(addtime == null){
											...
											if(System.currentTimeMillis()-addtime<48*60*60*1000)
											{
												...
												if(content != null && "1".equals(content)){
													resultStr = "errer";
													content = "该任务已经超过定时发送时间，审批失败！";
												}else if(content != null && "2".equals(content)){
													resultStr = "errer";
													content = "该任务已被撤销！";
												}else if(content != null && "3".equals(content)){
													resultStr = "errer";
													content = "审批失败！";
												}else if(content != null && "4".equals(content)){
													EmpExecutionContext.error("["+sdf.format(new Date())+"] 获取该任务对象失败！");
													resultStr = "fail";
												}
											}else{
				...
				EmpExecutionContext.error(e,"审批提醒手机上行审批出现异常！");
				return true;
			}finally{
	...
	private String judgeMotask(LfMotask moTask){
		...
		try{
			...
			returnmsg = "fail";
			EmpExecutionContext.error(e,"判断上行记录是否合法出现异常！");
		}
		return returnmsg;
	}
	...
	private LinkedHashMap<String,LfExamineSms> analysisMsg(String content,LfMotask moTask){
		try{
			...
			EmpExecutionContext.error(e,"解析上行内容出现异常！");
		}
		return null;
	}
	...
	private Long getAddTime(LfFlowRecord record){
		...
		try{
			...
			if(record.getRLevel() == 1){
				//短信 彩信      2014-1-10增加了网讯发送
				if(record.getInfoType() == 1 || record.getInfoType() == 2||record.getInfoType() == 5){
				//record.getMtId() taskid 取值错误导致短信没有审核 may modif 2013-12-27
//					LfMttask lfMttask = baseBiz.getById(LfMttask.class, record.getMtId());
					LfMttask lfMttask= new CommonBiz().getLfMttaskbyTaskId(record.getMtId());
					if(lfMttask != null){
...
//				}else if(record.getInfoType() == 6){//由于别的地方已经增加了判断此处去掉
//					LfWXBASEINFO lfWXBasinfo = empDao.findObjectByID(LfWXBASEINFO.class, record.getMtId());
//					if(lfWXBasinfo!=null){
//						addtime = lfWXBasinfo.getCREATDATE().getTime();
//					}
				}
			...
			addtime = null;
			EmpExecutionContext.error(e,"获取该任务 该模板 的创建时间出现异常！");
		}
		return addtime;
	}
	...
	/**
	 *
	 * @description    上行审批信息 对任务进行审批
	 * @param record	审批实时记录
	 * @return          1超、时  2 已被撤销 3审批失败！4查询任务失败			 	
	 * @author yejiangmin <282905282@qq.com>
	 * @datetime 2013-10-18 上午10:48:12
	 */
	private String examineMsgByType(LfFlowRecord record){
		...
		try{
			...
			if(record.getInfoType() == 1 || record.getInfoType() == 2||record.getInfoType() == 5){
				...
				if(lfMttask.getSubState() == 2){
					
					if (lfMttask.getTimerStatus() == 1){
						//判断定时时间是否大于当前时间
						if (lfMttask.getTimerTime().getTime() < Calendar.getInstance().getTime().getTime()+60*1000){
							return "1";
						}
					}
					MsgExamineBiz  examineBiz = new MsgExamineBiz();
					String isLastCheck = examineBiz.judgeLastChecker(record);;
					if(record.getInfoType() == 1||record.getInfoType() == 5){
						examineBiz.examineSMS(record, lfMttask.getTimerStatus(),
								String.valueOf(lfMttask.getTimerTime()), isLastCheck);
					}else if(record.getInfoType() == 2){
						examineBiz.examineMMS(record, lfMttask.getTimerStatus(),
								String.valueOf(lfMttask.getTimerTime()), isLastCheck);
					}
					returnmsg = getDownReviewContent(createteakuser, record.getInfoType(), record.getRState(), lfMttask, null);
				}else if(lfMttask.getSubState() == 3){
					//该任务已被撤销！
					returnmsg = "2";
				}
			}else if(record.getInfoType() == 3 || record.getInfoType() == 4){
				boolean isflag = new TempReviewBiz().reviewTemplate(record.getFrId(),record.getRState(),record.getComments());
				if(isflag){
			...
			EmpExecutionContext.error(e,"任务审批出现异常！");
		}
		return returnmsg;
	}
	...
	private String getDownReviewContent(LfSysuser createuser,Integer type,Integer isAuditFlag,
		...
		try{
			...
			EmpExecutionContext.error(e,"获取下行的内容出现异常！");
			msgContent = "";
		}
		return msgContent;
	}
	...
	private void sendDownResultMsg(LfMotask moTask,
		...
		try{
			...
			if(!"".equals(spUser) &&  spUser != null)
			{
				...
				ReviewRemindBiz remindBiz = new ReviewRemindBiz();
				...
				if("success".equals(errerStr)){
					if(examineSms!=null&&messageCount >0){
						...
						List<LfExamineSms> list = new ArrayList<LfExamineSms>();
						list.add(examineSms);
						remindBiz.sendMsgByMoblieStr(list, spUser, subno, messageCount,
								flag, "0", StaticValue.VERIFYREMIND_MENUCODE, moTask.getPhone(), content, adminUser);
					}
				}else if("errer".equals(errerStr)){
					remindBiz.sendMsgByMoblieStr(null, spUser, subno, messageCount,
							flag, "0", StaticValue.VERIFYREMIND_MENUCODE, moTask.getPhone(), content, adminUser);
				}
			}
		}catch (Exception e) {
			EmpExecutionContext.error(e,"下发信息出现异常！");
		}
	}
	...
	private String sendMsgToOne(LfExamineSms lfExamineSms,WGParams wgParams,Integer count,LfSysuser lfSysuser,boolean flag) {
		...
		if(flag){
			...
			try {
				...
				Integer resultMsg = new BalanceLogBiz().sendSmsAmountByGuid(conn, lfSysuser.getGuId(), count);
				if(resultMsg == 0){
					...
					if(wegGateResult == null || wegGateResult.length == 0){
						...
						if(lfExamineSms != null)
						{
							...
							empTransDao.update(conn, lfExamineSms);
						}
				...
				empTransDao.rollBackTransaction(conn);
				EmpExecutionContext.error(e,"下发信息失败！");
			}finally{
