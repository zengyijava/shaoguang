...
public class TmsFile {
	...
	private byte[] readFileContents(String fileName) {
		...
		try {
			...
			EmpExecutionContext.error(e,"读取文件信息出现异常！");
			return contentBytes;
		}finally{
			if(in != null)
			{
				...
				{
					EmpExecutionContext.error(e, "读取文件关闭字符输出流异常！");
				}
			}
		}
	...
	public String tmsToBase64(String tmsPath)
	{
		...
		{
			EmpExecutionContext.error(e, "模板文件编码异常！");
			return base64;
		}finally{
			if(in != null)
			{
				...
				{
					EmpExecutionContext.error(e, "模板文件关闭字符输出流异常！");
				}
			}
		}
