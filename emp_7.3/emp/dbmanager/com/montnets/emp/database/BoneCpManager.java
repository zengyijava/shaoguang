...
public class BoneCpManager {
	...
	private static BoneCpManager instance = new BoneCpManager();
	...
	public synchronized static BoneCpManager getInstance() {
		if (instance == null) {
	...
	private void getProInfo() {
		...
		try {
			...
			EmpExecutionContext.error("BONECP连接池,数据库名:"+databaseName+",数据库加密密码:"+ StaticValue.getDbPasswd());
	...
	private void createPool(String poolName) {
		...
		try {
			...
			config.setMaxConnectionsPerPartition(maxPoolSize);//设置每个分区含有connection最大个数

			config.setMinConnectionsPerPartition(minPoolSize);//设置每个分区含有connection最小个数
