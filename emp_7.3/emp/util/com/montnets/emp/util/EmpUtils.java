...
public class EmpUtils {
	...
	public static String getPartDepCode(String depCode) {
		...
		try {
			...
			partDepCode = depCode;
			EmpExecutionContext.error(e, "获取父机构编码异常，depCode：" + depCode);
		}
		return partDepCode;
	}
	/**
	...
	public static String getPartDepCode(String depCode,String defaultValue) {
		...
		try {
			//partDepCode = depCode.replaceAll("0+$", "");
			partDepCode = depCode.replaceAll("0+$", "");
			int length = partDepCode.length();
			int defaultLength = depCode.length();
			if("".equals(partDepCode) || length ==0){
				partDepCode = "";
			}else{
				 if(length%2 == 0){
	...
	public static String StringToBase64(String str){
		...
		{
			try {
				strBuffer.append(base64.encode(params[i].getBytes("GBK")).replaceAll("\r\n", "").replaceAll("\r", "").replaceAll("\n", "")).append(",");
			} catch (UnsupportedEncodingException e) {
				EmpExecutionContext.error(e, "转成BASE64编码的字符串使用GBK编码");
			}
		}
