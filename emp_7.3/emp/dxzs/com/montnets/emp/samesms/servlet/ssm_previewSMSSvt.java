...
public class ssm_previewSMSSvt extends BaseServlet {
    ...
    public void preview(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            if (strlguserid == null
                || strlguserid.trim().length() == 0
                || "null".equals(strlguserid.trim())
                || "undefined".equals(strlguserid.trim())) {
                EmpExecutionContext.error("相同内容预览，获取操作员id异常。userid:"
                                          + strlguserid
                                          + "，errCode："
                                          + IErrorCode.V10001);
            ...
            if (haoduan == null) {
                EmpExecutionContext.error("相同内容预览，获取运营商号码段异常。userId："
                                          + strlguserid
                                          + "，errCode:"
                                          + IErrorCode.V10002);
            ...
            if (phoneFilePath == null) {
                EmpExecutionContext.error("相同内容预览，获取发送文件路径失败。userId："
                                          + strlguserid
                                          + "，errCode:"
                                          + IErrorCode.V10013);
                ...
                if (new File(phoneFilePath[0]).exists()) {
                    EmpExecutionContext.error("相同内容预览，获取发送文件路径失败，文件路径已存在，文件路径："
                                              + phoneFilePath[0]
                                              + "，userid:"
                                              + strlguserid
                                              + "，errCode："
            ...
            try {
                ...
                // 捕获到文件超出最大数限制的异常
                EmpExecutionContext.error("相同内容预览，文件上传失败，超出上传文件大小限制。userId："
                                          + strlguserid
                                          + "，errCode:"
                                          + IErrorCode.V10014);
                ...
                logInfo.append("相同内容预览，表单流上传失败。userId:")
                       .append(strlguserid)
                       .append("，errCode:")
                       .append(IErrorCode.V10003)
                       .append("，耗时:")
            ...
            while (it.hasNext()) {
                ...
                if (!fileItem.isFormField() && fileItem.getName().length() > 0) {
                    ...
                    if (isOverSize) {
                        // 大小超出限制
                        EmpExecutionContext.error("相同内容预览，文件上传失败，单个zip文件超出上传文件大小限制。userId："
                                                  + strlguserid
                                                  + "，errCode:"
                                                  + IErrorCode.V10014);
            ...
            // 获取群组id
            groupStr = fieldInfo.get("groupIds");
            // 获取发送账号
            spUser = fieldInfo.get("spUser1");
            // 获取发送短信内容
            msg = fieldInfo.get("msg");
            // 大文件批次号
            String bigfileid = fieldInfo.get("bigfileid");
            boolean isbigfile = isbigfilesend(bigfileid);
            long bigfilemax = StaticValue.MAX_PHONE_NUM;
            String bigtaiskids = fieldInfo.get("bigtaiskids");
            // 获取短信贴尾内容
            ...
            // 登录操作员信息为空
            if (lfSysuser == null) {
                EmpExecutionContext.error("相同内容预览，从session获取登录操作员信息异常。lfSysuser为null，errCode："
                                          + IErrorCode.V10001);
                ErrorCodeInfo info = ErrorCodeInfo.getInstance(langName);
                String desc = info.getErrorInfo(IErrorCode.V10001);
                result = desc;
            ...
            boolean checkFlag = new CheckUtil().checkSysuserInCorp(lfSysuser,
                                                                   ...
                                                                   null);
            if (!checkFlag) {
                EmpExecutionContext.error("相同内容预览，检查操作员、企业编码、发送账号不通过，taskid:"
                                          + taskId
                                          + "，corpCode:"
                                          + lgcorpcode
                                          + "，userid："
            ...
            if (msg != null && msg.trim().length() > 0) {
                if (lgcorpcode != null && lgcorpcode.trim().length() > 0) {
                    ...
                    // 包含关键字
                    if (!"".equals(words)) {
                        EmpExecutionContext.error("相同内容预览，关键字检查未通过，发送内容包含违禁词组:'"
                                                  + words
                                                  + "'，taskid:"
                                                  + taskId
                                                  + "，corpCode:"
                    ...
                    EmpExecutionContext.error("相同内容预览，获取企业编码异常，lgcorpcode:"
                                              + lgcorpcode
                                              + "，taskid:"
                                              + taskId
                                              + "，userid："
                ...
                EmpExecutionContext.error("相同内容预览，获取短信内容异常，短信内容为空。taskid:"
                                          + taskId
                                          + "，corpCode:"
                                          + lgcorpcode
                                          + "，userid："
            ...
            if (fileItemsList != null && fileItemsList.size() > 0) {
                ...
                for (int i = 0; i < loadFileNameSingle.size(); i++) {
                    // 通过文件名截取到文件类型
                    String fileType = loadFileNameSingle.get(i)
                                                        .substring(loadFileNameSingle.get(i)
                                                                                     .lastIndexOf("."))
                                                        .toLowerCase();
                    // 判断文件类型
                    if (!fileType.equals(".rar")
                        ...
                        && !fileType.equals(".txt")) {
                        // 文件类型不合法
                        EmpExecutionContext.error("相同内容预览，文件上传失败，文件类型不合法。userId："
                                                  + strlguserid
                                                  + "，errCode:"
                                                  + ErrorCodeInfo.V10003);
            ...
            if (containDraft != null && containDraft.trim().length() > 0) {
                ...
                int draftRes = smsAtom.setDraftReader(readerList,
                                                      draftId,
                                                      draftFileTemp,
                                                      lguserid.toString(),
                                                      lgcorpcode);
                ...
                // 查不到草稿箱对象
                    ...
                    result = "draftnotex";
                    EmpExecutionContext.error("相同内容预览，设置草稿箱文件流失败，获取不到草稿箱对象，draftId="
                                              + draftId
                                              + "，taskid:"
                                              + taskId
                                              + "，corpCode:"
                ...
                // 草稿箱文件不存在
                    ...
                    result = "draftnotfile";
                    EmpExecutionContext.error("相同内容预览，设置草稿箱文件流失败，获取不到草稿箱文件对象，文件不存在，draftId="
                                              + draftId
                                              + "，taskid:"
                                              + taskId
                                              + "，corpCode:"
                ...
                // 其他失败情况
                    ...
                    result = "drafterr";
                    EmpExecutionContext.error("相同内容预览，设置草稿箱文件流失败，返回状态："
                                              + draftRes
                                              + "，draftId="
                                              + draftId
                                              + "，taskid:"
            ...
            // 判断有效号码数是否超出范围
            if (preParams.getEffCount() > bigfilemax) {
                // 删除前面生成的有效号码和无效号码文件
                ...
                // 文件内有效号码大于500万
                result = "overstep";
                EmpExecutionContext.error("相同内容预览，有效号码数超出过最大限制:"
            ...
            // 解析手工输入和通讯录的号码
            smsAtom.parseAddrAndInputPhone(preParams,
                                           depIdStr,
                                           phoneStr,
                                           groupStr,
                                           lgcorpcode,
            ...
            if (isbigfile) {
                // 重新整理预览参数
                repair(preParams, bigfileid, bigtaiskids, objMap);
            ...
            if (preParams.getSubCount() == 0) {
                result = "noPhone";
                EmpExecutionContext.error("相同内容预览，没有提交号码，无法进行发送。taskid:"
                                          + taskId
                                          + "，corpCode:"
                                          + lgcorpcode
                                          + "，userid："
            ...
            if (preSendCount < 0) {
                EmpExecutionContext.error("相同内容预览，获取预发送条数异常。errCode："
                                          + IErrorCode.B20006
                                          + "，taskid:"
                                          + taskId
                                          + "，corpCode:"
            ...
            if (preSendCount == 0) {
                result = "noPhone";
                EmpExecutionContext.error("相同内容预览，没有可发送的有效号码，无法进行发送。taskid:"
                                          + taskId
                                          + "，corpCode:"
                                          + lgcorpcode
                                          + "，userid："
            ...
            // 判断是否需要机构计费
            Map<String, String> infoMap = new CommonBiz().checkMapNull(null, lguserid, lgcorpcode);
            ...
            if (isCharge) {
                ...
                if (depFeeCount == null || depFeeCount < 0) {
                    EmpExecutionContext.error("相同内容预览，获取机构余额异常。errCode："
                                              + IErrorCode.B20024
                                              + "，taskid:"
                                              + taskId
                                              + "，corpCode:"
                ...
                if (depFeeCount - preSendCount < 0) {
                    // 设置检查标识为false余额不足
                    isAllCharge = false;
                    EmpExecutionContext.error("相同内容预览，机构余额小于发送条数，taskid:"
                                              + taskId
                                              + "，corpCode:"
                                              + lgcorpcode
                                              + "，userid："
            ...
            if (isAllCharge) {
                ...
                if (feeFlag == null || feeFlag < 0) {
                    EmpExecutionContext.error("相同内容预览，获取SP账号计费类型异常。errCode："
                                              + IErrorCode.B20044
                                              + "，taskid:"
                                              + taskId
                                              + "，corpCode:"
                ...
                if (feeFlag == 1) {
                    ...
                    if (spUserFeeCount == null || spUserFeeCount < 0) {
                        EmpExecutionContext.error("相同内容预览，获取SP账号余额异常。errCode："
                                                  + IErrorCode.B20045
                                                  + "，taskid:"
                                                  + taskId
                                                  + "，corpCode:"
                    ...
                    if (spUserFeeCount - preSendCount < 0) {
                        // 设置检查标识为false余额不足
                        isAllCharge = false;
                        EmpExecutionContext.error("相同内容预览，SP账号余额小于发送条数。，taskid:"
                                                  + taskId
                                                  + "，corpCode:"
                                                  + lgcorpcode
                                                  + "，userid："
            ...
            if (phoneArr.length > 0 &&!"".equals(preParams.getPreviewPhone())) {
                for (String phone : phoneArr) {
                    ...
                    if (null != xtGateQueue) {
                    	// 优先显示中文签名，其次显示英文签名
                        String sign = (null != xtGateQueue.getSignstr()&&!"".equals(xtGateQueue.getSignstr().trim()) ? xtGateQueue.getSignstr() : (null != xtGateQueue.getEnsignstr() &&!"".equals(xtGateQueue.getEnsignstr().trim()) ? xtGateQueue.getEnsignstr() : ""));
                        // 签名是否前置
            ...
            preParams.setPreviewPhone(phoneSign.substring(0, phoneSign.length() - 1));

            // 预览信息 json
            result = preParams.getJsonStrBigfile(objMap, isbigfile);

            // 操作日志信息
            StringBuffer opContent = new StringBuffer().append("预览S：")
                                                       ...
                                                       .append(fileCount)
                                                       .append("，总大小:")
                                                       .append(allFileSize / 1024)
                                                       .append("KB");
            // 记录上传文件名
            if (fileCount > 0) {
                opContent.append("，文件名：")
                         .append(loadFileName.deleteCharAt(loadFileName.length() - 1).append("'"));
            }
            // 选择机构
            if (depIdStr != null && depIdStr.length() > 1) {
                opContent.append("，DEPID：").append(depIdStr.substring(1, depIdStr.length() - 1));
            }
            // 选择群组
            if (groupStr != null && groupStr.length() > 1) {
                opContent.append("，群组ID：").append(groupStr.substring(1, groupStr.length() - 1));
            }

            opContent.append("，提交总数：")
                     .append(preParams.getSubCount())
                     .append("，有效数：")
                     .append(preParams.getEffCount())
                     .append("，短信条数：")
                     .append(preSendCount)
                     .append("，taskId：")
                     .append(taskId)
            ...
            request.getRequestDispatcher(this.empRoot + "/samesms/ssm_preview.jsp")
                   .forward(request, response);
        }
    }

    private boolean repair(PreviewParams preParams,
                           String bigfileid,
                           String bigtaiskids,
        ...
        if (bigfileid != null) {
            ...
            try {
                LfBigFile lfbf = new BaseBiz().getById(LfBigFile.class, bigfileid);
                String str = lfbf.getOprNum();
                int[] opr = StringToInt(str.split(","));
                preParams.setEffCount(lfbf.getEffCount().intValue());
                preParams.setPreviewPhone(lfbf.getViewUrl());
                preParams.setBlackCount(lfbf.getBlaCount().intValue());
                preParams.setOprValidPhone(opr);
                preParams.setSubCount(lfbf.getSubCount().intValue());
                preParams.setRepeatCount(lfbf.getRepCount().intValue());
                preParams.setBadCount(lfbf.getErrCount().intValue()
                                      + lfbf.getRepCount().intValue()
                                      + lfbf.getBlaCount().intValue());
                preParams.setBadModeCount(lfbf.getErrCount().intValue());
                objMap.put("viewFilePath", lfbf.getBadUrl());
            }
            catch (Exception e) {
    ...
    public void toDraft(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            // 登录操作员信息为空
            if (lfSysuser == null) {
                EmpExecutionContext.error("相同内容暂存草稿，从session获取登录操作员信息异常。lfSysuser为null，errCode："
                                          + IErrorCode.V10001);
                ErrorCodeInfo info = ErrorCodeInfo.getInstance(langName);
                String desc = info.getErrorInfo(IErrorCode.V10001);
                result = desc;
            ...
            String[] phoneFilePath = txtFileUtil.getSaveUrl(lguserid, StaticValue.FILEDIRNAME
                                                                      + "drafttxt/ssm_sendbatch");
            if (phoneFilePath == null || phoneFilePath.length < 2) {
                EmpExecutionContext.error("相同内容暂存草稿，获取发送文件路径失败。userId："
                                          + strlguserid
                                          + "，errCode:"
                                          + IErrorCode.V10013);
                throw new EMPException(IErrorCode.V10013);
                ...
                // 判断文件是否存在，存在则返回
                if (new File(phoneFilePath[0]).exists()) {
                    EmpExecutionContext.error("相同内容暂存草稿，发送文件路径已存在，文件路径："
                                              + phoneFilePath[0]
                                              + "，userid:"
                                              + strlguserid
                                              + "，errCode："
            ...
            try {
                ...
                // 捕获到文件超出最大数限制的异常
                EmpExecutionContext.error("相同内容暂存草稿，超出上传文件大小限制。userId："
                                          + strlguserid
                                          + "，errCode:"
                                          + IErrorCode.V10014);
                throw new EMPException(IErrorCode.V10014, e);
                ...
                StringBuffer logInfo = new StringBuffer();
                logInfo.append("相同内容暂存草稿，表单流上传失败。userId:")
                       .append(strlguserid)
                       .append("，errCode:")
                       .append(IErrorCode.V10003)
                       .append("，耗时:")
            ...
            while (it.hasNext()) {
                ...
                if (!fileItem.isFormField() && fileItem.getName().length() > 0) {
                    ...
                    if (isOverSize) {
                        // 大小超出限制
                        EmpExecutionContext.error("相同内容暂存草稿，单个zip文件超出上传文件大小限制。userId："
                                                  + strlguserid
                                                  + "，errCode:"
                                                  + IErrorCode.V10014);
                        throw new EMPException(IErrorCode.V10014);
            ...
            if (draftId != null && draftId.trim().length() > 0) {
                ...
                if (oldDrafts == null && containDraft != null && containDraft.trim().length() > 0) {
                    // 草稿箱不存在
                    result = "draftnotex";
                    EmpExecutionContext.error("相同内容暂存草稿，草稿箱不存在，draftId："
                                              + draftId
                                              + "，taskid:"
                                              + taskId
                                              + "，corpCode:"
            ...
            if (preParams.getEffCount() > StaticValue.MAX_PHONE_NUM) {
                ...
                // 文件内有效号码大于500万
                result = "overstep";
                EmpExecutionContext.error("相同内容暂存草稿，有效号码数超过"
                                          + StaticValue.MAX_PHONE_NUM
                                          + "，taskid:"
                                          + taskId
                                          + "，corpCode:"
            ...
            if (draftId == null || draftId.trim().length() < 1) {
                ...
                // 加载存在的草稿箱对象
                // LfDrafts oldDrafts = baseBiz.getById(LfDrafts.class,
                // drafts.getId());
                draftFilePath = oldDrafts.getMobileurl();
            }

            // 设置草稿箱号码url
            smsAtom.setDraftMobileUrl(drafts,
                                      preParams,
                                      containDraft,
                                      draftFilePath,
                                      draftFileTemp,
            ...
            // 操作日志信息
            StringBuffer opContent = new StringBuffer().append("相同内容暂存草稿S：")
                                                       .append(sdf.format(startTime))
                                                       .append("，耗时：")
                                                       .append((System.currentTimeMillis() - startTime))
                                                       .append("ms，文件数:")
                                                       ...
                                                       .append("，草稿箱名称：")
                                                       .append(drafts.getTitle())
                                                       .append("，草稿箱文件url：")
                                                       .append(drafts.getMobileurl());
            // 记录上传文件名
            if (fileCount > 0) {
                opContent.append("，文件名：")
                         .append(loadFileName.deleteCharAt(loadFileName.length() - 1).append("'"));
            ...
            preParams = null;
            request.setAttribute("result", result);
            request.getRequestDispatcher(this.empRoot + "/samesms/ssm_drafts.jsp")
                   .forward(request, response);
        }
    }

    private boolean isbigfilesend(String bigfileid) {
