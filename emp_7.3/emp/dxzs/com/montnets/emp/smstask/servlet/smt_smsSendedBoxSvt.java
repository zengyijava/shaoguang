package com.montnets.emp.smstask.servlet;
...
import com.montnets.emp.smstask.biz.ReplyParams;
import com.montnets.emp.smstask.biz.SmstaskBiz;
import com.montnets.emp.smstask.biz.SmstaskExcelTool;
...
public class smt_smsSendedBoxSvt extends BaseServlet {
    ...
    //模板路径
    protected final String excelPath = path + "dxzs/smstask/file/";

    //true代表正在汇总 fasle代表已汇总
    private static boolean SUMMARYING_FLAG = false;
    //当前正在汇总的taskId
    private static String SUMMARYING_TASKID = "0";
    ...
    final SimpleDateFormat sdf_yyyyMM = new SimpleDateFormat("yyyyMM");


    public void getProgress(HttpServletRequest request, HttpServletResponse response) {
        String taskId = request.getParameter("taskId");
        com.alibaba.fastjson.JSONObject rsObject = smstaskBiz.getProgress(taskId);
    ...
    public void find(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            if (isBack) {
                if (session.getAttribute("smt_smsSended_pageinfo") != null) {
                    pageInfo = (PageInfo) session.getAttribute("smt_smsSended_pageinfo");
                }
                ;
            ...
            //获取过滤条件
            userid = (userid != null && userid.trim().length() > 0 && !userid.equals("请选择") && userid.indexOf(",") != -1 && userid.endsWith(",")) ? userid.substring(0, userid.lastIndexOf(",")) : "";
...
//					  String sendstate=request.getParameter("sendstate");
     ...
     * @param response
     */
    public void summaryByTaskid(HttpServletRequest request, HttpServletResponse response) throws IOException {
        String taskId = "0";
        try {
            //任务批次查询条件
            ...
            //汇总中
            if(SUMMARYING_FLAG){
                EmpExecutionContext.info("群发历史任务正在汇总，请勿重复执行！");
                response.getWriter().print("executing," + SUMMARYING_TASKID);
                return;
            }
            SUMMARYING_TASKID = taskId;
            SUMMARYING_FLAG = true;
            //获取页面传过来的lf_mttask表中的mtid
            String mtid = request.getParameter("mtid");
            long startTime= System.currentTimeMillis();
            EmpExecutionContext.info("群发历史任务汇总开始！");
            boolean result = smstaskBiz.updateLfmttask(taskId, mtid);
            long endTime = System.currentTimeMillis();
            long costTime = endTime - startTime;
            EmpExecutionContext.info("群发历史任务汇总结束，耗时：" + costTime);
            SUMMARYING_FLAG = false;
            SUMMARYING_TASKID = "0";
            response.getWriter().print(result + "" + taskId);
        } catch (Exception e) {
            EmpExecutionContext.error(e, "群发历史任务汇总异常！");
            SUMMARYING_FLAG = false;
            SUMMARYING_TASKID = "0";
            response.getWriter().print("false" + taskId);
        }
    }
            ...
            if (mt != null) {
                ...
                if (replyDetailList != null && replyDetailList.size() > 0) {
                    ...
                    for (int i = 0; i < replyDetailList.size(); i++) {
                        ...
                        member.put("time", time.indexOf(".") > 0 ? time.substring(0, time.lastIndexOf(".")) : time);
            ...
            if (mt != null) {
                ...
                if (replyDetailList != null && replyDetailList.size() > 0) {
                    ...
                    for (int i = 0; i < replyDetailList.size(); i++) {
                        ...
                        //回复时间
                        time = replyDetai.get("delivertime") != null ? replyDetai.get("delivertime").toString() : "-";
                        replyParams.setTime(time.indexOf(".") > 0 ? time.substring(0, time.lastIndexOf(".")) : time);
...
//					EmpExecutionContext.info("群发历史查询", corpCode, lguserid, opUser, "群发历史查询回复详情记录导出，共导出"+ReplyParamsList.size()+"条记录。", "OTHER");
     ...
     * @param response
     */
    public void toChangeTiming(HttpServletRequest request, HttpServletResponse response) {
        String mtId = request.getParameter("mtId");
        String bmtType = request.getParameter("bmtType");
        String msgType = request.getParameter("msgType");
        request.setAttribute("mtId", mtId);
        //request.setAttribute("bmtType", bmtType);
        request.setAttribute("msgType", msgType);
        LfMttask lfMttask = smstaskBiz.findLfMttaskById(Long.valueOf(mtId));

        LfSysuser lfSysuser = (LfSysuser) request.getSession(false).getAttribute("loginSysuser");
        String lgcorpcode = lfSysuser.getCorpCode();
        String tailcontents = new SmsSendBiz().getTailContents(lfMttask.getBusCode(), lfMttask.getSpUser(), lgcorpcode);
        String msg = lfMttask.getMsg();
        if (null != msg && StringUtils.isNotBlank(tailcontents)) {
            if (msg.lastIndexOf(tailcontents) != -1) {
                lfMttask.setMsg(lfMttask.getMsg().substring(0, lfMttask.getMsg().lastIndexOf(tailcontents)));
            }
        }
        request.setAttribute("mtMsg", lfMttask.getMsg());
        request.setAttribute("busCode", lfMttask.getBusCode());
        request.setAttribute("spUser", lfMttask.getSpUser());
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        request.setAttribute("timerTime", sdf.format(lfMttask.getTimerTime()));
        try {
            String path = "/smt_changeTask.jsp";
            request.getRequestDispatcher(empRoot + basePath + path)
                    .forward(request, response);
        } catch (Exception e) {
            EmpExecutionContext.error(e, "转发异常");
        }
    }
     ...
     * @param response
     */
    public void changeTiming(HttpServletRequest request, HttpServletResponse response) {

        String mtId = request.getParameter("mtId");
        String timerTime = request.getParameter("timerTime");
        String mtMsg = request.getParameter("mtMsg");
        //贴尾
        String showtailcontent = request.getParameter("showtailcontent");
        LfSysuser lfSysuser = (LfSysuser) request.getSession(false).getAttribute(StaticValue.SESSION_USER_KEY);
        String result = smstaskBiz.changeTiming(mtId, timerTime, mtMsg, showtailcontent, lfSysuser.getUserId(), lfSysuser.getCorpCode(), request);
        try {
            response.getWriter().print(result);
        } catch (IOException e) {
            EmpExecutionContext.error(e, "获取PrintWriter异常");
        }
    }
    ...
    private final SmstaskDAO smsTaskDao = new SmstaskDAO();
     ...
     */
    @SuppressWarnings("unchecked")
    public void ssm_preview(HttpServletRequest request,
                            HttpServletResponse response) throws Exception {
        long startTime = System.currentTimeMillis();
        //返回结果
        String result = "";
        ...
        //重发类型
        String resendType = request.getParameter("resendType");
        //获取页面传过来的lf_mttask表中的mtid
        String encryptmtid = request.getParameter("mtid");
        //获取加密类
        ParamsEncryptOrDecrypt encryptOrDecrypt = (ParamsEncryptOrDecrypt) request.getSession(false).getAttribute("decryptObj");
...
//			String taskId = fieldInfo.get("taskId");
            ...
            //resendType 重发类型  1：提交失败重发;2：接收失败重发;3：选中重发
            //3：选中重发
            if ("3".equals(resendType)) {
                ...
                //相同内容重发获取号码
                List<SendedMttaskVo> mttaskvoList = smsTaskDao.getReSendPhone(resendType, mtid);
                //相同内容短信存在拆分，需要过滤重复号码
                for (int i = 0; i < mttaskvoList.size(); i++) {
                    // 添加号码，利用HashSet特性，添加的对象与集合中不存在就添加，存在就覆盖。
                    sendPhoneSet.add(mttaskvoList.get(i).getPhone());
                }
            }
            ...
            if (preParams.getEffCount() > StaticValue.MAX_PHONE_NUM) {
                ...
                //文件内有效号码大于500万
                result = "overstep";
                EmpExecutionContext.error("相同内容预览，有效号码数超出过最大限制:" + StaticValue.MAX_PHONE_NUM + "，无法进行发送，taskid:" + taskId + "，corpCode:" + lgcorpcode + "，userid：" + strlguserid);
                return;
            }
            ...
            if (phoneArr.length > 0) {
                for (String phone : phoneArr) {
                    ...
                    // 是否配置了对应运营商的通道
                    if (null != xtGateQueue) {
                        String sign = (null == xtGateQueue.getSignstr() ? "" : xtGateQueue.getSignstr());
                        // 签名是否前置
                        ...
                        if ("0".equals(spisuncm)) {
                            ...
                            phoneSign += phoneNumber + splitChar + "非法号码" + splitChar + splitChar + ";";
                        }
                    } else {
                        if ("0".equals(spisuncm)) {
            ...
            request.getRequestDispatcher(
                    this.empRoot + "/dxzs/smstask/smt_reSendPreview.jsp").forward(request,
                    response);
        }
    }
    ...
    public void readFile(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            readStr = CommonSvt.unescape(readStr);
            response.setContentType("text/plain;charset=GBK");
            response.getOutputStream().write(readStr.getBytes("GBK"));
