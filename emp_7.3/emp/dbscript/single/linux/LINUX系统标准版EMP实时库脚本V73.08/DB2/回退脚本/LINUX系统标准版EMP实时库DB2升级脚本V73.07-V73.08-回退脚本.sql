...
CREATE OR REPLACE PROCEDURE "ADDVERSIONEXISTS"
  BEGIN ATOMIC
    ...
    DECLARE NUM INT;
    DECLARE TOTAL INT;
    SET VERSIONSTR = '7.3.7.607.SP7';
    SET DBVERSION = '73.07';
    SET WBSVERSION = '8.6.4.219';
    SET SPGATEVERSION = '6.1.52.344';
    SET VERSIONSTR_OLD = '7.3.8.612.SP8';
    SET NUM =1;
    SET TOTAL=1;
    --EMP产品版本记录
    --EMP-WEB
    ...
    THEN
      ...
      VALUES(1000,1000,VERSIONSTR,CURRENT TIMESTAMP,CURRENT TIMESTAMP,'EMP-WEB',1,'1.修复查询统计-统计报表，SP账号/机构/操作员/业务类型统计报表，页面查询不出数据<br/>2.修复移动办公-员工通讯录管理-员工通讯录，员工手机号码修改不生效');
    ELSE
      UPDATE LF_VERSION_HIS SET UPDATETIME=CURRENT TIMESTAMP,ISRELEASE=1,VERSIONINFO='1.修复查询统计-统计报表，SP账号/机构/操作员/业务类型统计报表，页面查询不出数据<br/>2.修复移动办公-员工通讯录管理-员工通讯录，员工手机号码修改不生效' WHERE VERSION=VERSIONSTR AND PRODUCT_ID=1000 AND PROCESS_ID=1000;
    END IF;
    -- 删除旧版本web历史记录
