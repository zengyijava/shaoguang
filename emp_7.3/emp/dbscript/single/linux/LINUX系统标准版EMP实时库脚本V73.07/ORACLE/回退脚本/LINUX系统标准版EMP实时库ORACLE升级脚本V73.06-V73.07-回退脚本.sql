...
DECLARE COUNTNUM INT;
        ...
        TOTALINT INT;
BEGIN
    VERSIONSTR:='7.3.6.595.SP6';
    DBVERSIONSTR:='73.06';
    WBSVERSIONSTR:='8.6.4.219';
    SPGATEVERSIONSTR:='6.1.52.344';
    VERSIONSTR_OLD:='7.3.7.607.SP7';
  NUMNO:=1;
  TOTALINT:=1;
  --EMP产品版本记录
  ...
  IF COUNTNUM=0 THEN
    ...
    VALUES(1000,1000,VERSIONSTR,SYSDATE,SYSDATE,'EMP-WEB',1,'1、RCOS平台企业侧模板同步线程中模板的json 数据解析异常容错处理');
  ELSE
    UPDATE LF_VERSION_HIS SET UPDATETIME=SYSDATE,ISRELEASE=1,VERSIONINFO='1、RCOS平台企业侧模板同步线程中模板的json 数据解析异常容错处理' WHERE VERSION=VERSIONSTR AND PRODUCT_ID=1000 AND PROCESS_ID=1000;
  END IF;
  COMMIT;
    --删除EMP旧版本信息
