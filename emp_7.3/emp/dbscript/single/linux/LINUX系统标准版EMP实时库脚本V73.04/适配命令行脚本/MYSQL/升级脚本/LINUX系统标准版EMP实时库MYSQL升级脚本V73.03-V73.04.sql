...
BEGIN
    ...
    VALUES(DBVERSIONSTR,NOW(),NOW(),NUM,TOTALSTR,1,'EMP网关更改');
    END IF;
END     $      
...
BEGIN
	...
	ELSE
			UPDATE LF_VERSION_HIS SET UPDATETIME=NOW(),ISRELEASE=1,VERSIONINFO='1、修改空上行直接丢弃，不写库。<br />2、采用odbc绑定变量的方式读取滞留表。<br />3、修改API接口入口时短信内容被截断的问题。<br />4、修改api5.3接口下整形模块id无法识别的问题。<br />' WHERE PRODUCT_ID=1000 AND PROCESS_ID=1000;
	END IF;
	/*EMP_GATEWAY*/
	...
	END IF;
END     $
CALL LF_PVERV0('7.3.4.569.SP4','73.04','8.6.3.218','6.1.51.343',1,1) $
DROP PROCEDURE IF EXISTS `LF_PVERV0` $
/*****V73.03-V73.04 END*/