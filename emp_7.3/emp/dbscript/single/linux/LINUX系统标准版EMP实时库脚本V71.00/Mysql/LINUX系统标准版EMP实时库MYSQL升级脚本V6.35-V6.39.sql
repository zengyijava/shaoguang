...
END           
;; DELIMITER ;   
CALL LF_PVERV0('6.39',1,1);
DROP PROCEDURE IF EXISTS `LF_PVERV0`;

UPDATE LF_VERSION_HIS SET ISRELEASE=0 WHERE VERSION='6.6.0.343';

/*WEB升级脚本(V6.6.0.343-V6.7.0.360)START*/
DELIMITER ;;
...
/*WEB升级脚本(V6.6.0.343-V6.7.0.360)END*/

/*网关升级脚本(V4.05.07-V4.05.08)START*/

-- 增加字段，支持国际短信
DELIMITER ;;
DROP PROCEDURE IF EXISTS COLUMNADD; 
CREATE DEFINER=`root`@`%` PROCEDURE `COLUMNADD`(TBNAME VARCHAR(32),COLNAME VARCHAR(32),COLTYPE VARCHAR(64))
BEGIN
DECLARE  CURRENTDATABASE VARCHAR(100);
DECLARE  P_STR VARCHAR(300);
SELECT DATABASE() INTO CURRENTDATABASE;
...
CALL COLUMNADD('A_GWPARAMCONF','HKPARAMNAME','VARCHAR(256) NOT NULL  DEFAULT ''''');
CALL COLUMNADD('A_GWPARAMCONF','HKPARAMMEMO','VARCHAR(512) NOT NULL  DEFAULT ''''');

CALL COLUMNADD('A_GWPARAMCONF','ENPARAMNAME','VARCHAR(256) NOT NULL  DEFAULT ''''');
CALL COLUMNADD('A_GWPARAMCONF','ENPARAMMEMO','VARCHAR(512) NOT NULL  DEFAULT ''''');
...
UPDATE A_GWPARAMCONF SET HKPARAMNAME='滑動窗口大小(1-256)'                   ,HKPARAMMEMO='滑動視窗大小(非同步提交MO)(1-256)'                                                                                                , ENPARAMMEMO='Sliding Window size (Asynchronous commit MO)(1-32)'                                                                                                                                                                                                                    , ENPARAMNAME='sliding-window size(1-256)'                                   where paramitem='ASYNWINSIZEMON';      
UPDATE A_GWPARAMCONF SET HKPARAMNAME='MT滑動窗口大小(1-256)'                 ,HKPARAMMEMO='滑動視窗大小(非同步提交MT)(1-256)'                                                                                                , ENPARAMMEMO='Sliding Window size (Asynchronous commit MT)(1-32)'                                                                                                                                                                                                                    , ENPARAMNAME='MT sliding-window size(1-256)'                                where paramitem='ASYNWINSIZEMTN';      
...
CALL S_WR_VERINFO('4.05.08');
/*网关升级脚本(V4.05.07-V4.05.08)END*/

/*网关升级脚本(V4.05.08-V4.06.01)START*/
REPLACE INTO A_GWPARAMCONF(PARAMITEM, PARAMNAME,  PARAMMEMO,PARAMATTRIBUTE, VALUERANGE,
...
/*网关升级脚本(V4.05.08-V4.06.01)END*/

/*网关升级脚本(V4.06.01-V4.06.02)START*/
-- 更新配置参数，支持国际短信
UPDATE A_GWPARAMCONF SET HKPARAMNAME='流水號CheckPoint頻率'                  ,HKPARAMMEMO='多長時間進行一次checkpoint(單位:小時)'                                                                                            , ENPARAMMEMO='How long does it take to have a checkpoint (unit: hour), 2 hours by default, range of value: 1-24'                                                                                                                                                                     , ENPARAMNAME='Serial number CheckPoint frequency'                          where paramitem='6CLU1CHKPOINT';       
UPDATE A_GWPARAMCONF SET HKPARAMNAME='本地資料檔案目錄'                      ,HKPARAMMEMO='定時資料檔案輸出的本地路徑(填寫絕對路徑)，預設為空，取程式當前路徑'                                                               , ENPARAMMEMO='The local path of the timing data file output (Please use the absolute path), the default is empty, and the current path of the program is taken'                                                                                                                      , ENPARAMNAME='Local data file directory'                                   where paramitem='6CLU1DATAFILELPATH';  
UPDATE A_GWPARAMCONF SET HKPARAMNAME='共用資料檔案目錄'                      ,HKPARAMMEMO='集群閘道資料檔案共用路徑(填寫絕對路徑)，預設為空，不進行檔共用'                                                                   , ENPARAMMEMO='The cluster gateway data file sharing path (absolute path), the default is empty, and no file sharing is performed'                                                                                                                                                    , ENPARAMNAME='Shared data file directory'                                  where paramitem='6CLU1DATAFILERPATH';  
...
CALL S_WR_VERINFO('4.06.02');
/*网关升级脚本(V4.06.01-V4.06.02)END*/

/*网关升级脚本(V4.06.02-V4.06.03)START*/
...
CALL S_WR_VERINFO('4.06.03');
/*网关升级脚本(V4.06.02-V4.06.03)END*/

/* 网关升级脚本V4.06.03-V4.06.04 START */
REPLACE INTO a_gwparamconf (GWTYPE, PARAMITEM, PARAMNAME, PARAMATTRIBUTE, PARAMMEMO, DEFAULTVALUE, VALUERANGE, CONTROLTYPE, HKPARAMNAME, HKPARAMMEMO, ENPARAMNAME, ENPARAMMEMO) 
VALUES (3000, 'MTBUFSIZEN', 'MT接收缓冲', 1, 'MT接收缓冲，取值范围（10-50000）', '50000', '10-50000', 0, 'MT接收緩衝', 'MT接收緩衝，取值範圍（10-50000）', 'MT receive buffer', 'MT receive buffer Value range 10 - 50000');
...
BEGIN
		...
		IF NOT EXISTS(SELECT * FROM LF_VERSION_HIS WHERE VERSION=VERSIONSTR AND PRODUCT_ID=1000 AND PROCESS_ID=1000) THEN
			...
			VALUES(1000,1000,VERSIONSTR,NOW(),NOW(),'EMP-WEB',1,'1、网关性能优化。<br />');
		ELSE
			UPDATE LF_VERSION_HIS SET UPDATETIME=NOW(),ISRELEASE=1,VERSIONINFO='1、网关性能优化。<br />' WHERE VERSION=VERSIONSTR AND PRODUCT_ID=1000 AND PROCESS_ID=1000;
		END IF;
		/*EMP_GATEWAY*/
...
END;;    
DELIMITER ;   
CALL LF_PVERV0('6.7.0.371','6.39','8.5.0.198','6.1.30.322',1,1);
DROP PROCEDURE IF EXISTS `LF_PVERV0`;
/*****V6.35-V6.39 END*/   