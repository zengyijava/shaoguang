...
/*WEB升级脚本(V6.6.0.343-V6.7.0.371)START*/
...
/*WEB升级脚本(V6.6.0.343-V6.7.0.371)END*/



/*网关升级脚本START*/
/*V4.05.07-V4.05.08START*/
CREATE PROCEDURE "DELPROC"(IN "PROCNAME1" VARCHAR(20)) 
  begin
DECLARE STR VARCHAR (256);
DECLARE STR1 VARCHAR (256);
...
/*V4.05.07-V4.05.08 END*/

/*V4.05.08-V4.05.09 START*/
CREATE PROCEDURE "DELPROC"(IN "PROCNAME1" VARCHAR(20)) 
  begin
DECLARE STR VARCHAR (256);
DECLARE STR1 VARCHAR (256);
...
CALL ADDCOLUMN('A_GWPARAMCONF','HKPARAMNAME','VARCHAR(256) NOT NULL  DEFAULT ''''');
CALL ADDCOLUMN('A_GWPARAMCONF','HKPARAMMEMO','VARCHAR(512) NOT NULL  DEFAULT ''''');

CALL ADDCOLUMN('A_GWPARAMCONF','ENPARAMNAME','VARCHAR(256) NOT NULL  DEFAULT ''''');
CALL ADDCOLUMN('A_GWPARAMCONF','ENPARAMMEMO','VARCHAR(512) NOT NULL  DEFAULT ''''');

reorg table A_GWPARAMCONF;
...
/*V4.05.08-V4.05.09 END*/

/*V4.05.09-V4.06.01 START*/
        
MERGE INTO  A_GWPARAMCONF A USING (SELECT 'MORPTPUSHWAY' AS "PARAMITEM",4000 AS "GWTYPE" FROM  SYSIBM.DUAL) B ON (A.PARAMITEM=B.PARAMITEM AND A.GWTYPE=B.GWTYPE)
...
/*V4.05.09-V4.06.01 END*/

/*V4.06.01-V4.06.02 START*/
-- 更新配置参数，支持国际短信
UPDATE A_GWPARAMCONF SET HKPARAMNAME='流水號CheckPoint頻率'                  ,HKPARAMMEMO='多長時間進行一次checkpoint(單位:小時)'                                                                                            , ENPARAMMEMO='How long does it take to have a checkpoint (unit: hour), 2 hours by default, range of value: 1-24'                                                                                                                                                                     , ENPARAMNAME='Serial number CheckPoint frequency'                          where paramitem='6CLU1CHKPOINT';       
UPDATE A_GWPARAMCONF SET HKPARAMNAME='本地資料檔案目錄'                      ,HKPARAMMEMO='定時資料檔案輸出的本地路徑(填寫絕對路徑)，預設為空，取程式當前路徑'                                                               , ENPARAMMEMO='The local path of the timing data file output (Please use the absolute path), the default is empty, and the current path of the program is taken'                                                                                                                      , ENPARAMNAME='Local data file directory'                                   where paramitem='6CLU1DATAFILELPATH';  
UPDATE A_GWPARAMCONF SET HKPARAMNAME='共用資料檔案目錄'                      ,HKPARAMMEMO='集群閘道資料檔案共用路徑(填寫絕對路徑)，預設為空，不進行檔共用'                                                                   , ENPARAMMEMO='The cluster gateway data file sharing path (absolute path), the default is empty, and no file sharing is performed'                                                                                                                                                    , ENPARAMNAME='Shared data file directory'                                  where paramitem='6CLU1DATAFILERPATH';  
...
END IF;
END ;      
/*V4.06.01-V4.06.02 END*/


/*V4.06.02-V4.06.03 START*/
 CREATE PROCEDURE "DELFUNC"(IN "FUNCNAME1" VARCHAR(20)) 
  begin
DECLARE STR VARCHAR (256);
DECLARE STR1 VARCHAR (256);
SELECT  FUNCNAME INTO STR1 FROM SYSCAT.FUNCTIONS WHERE FUNCNAME=FUNCNAME1;
if  STR1 is not null THEN
  SET STR ='DROP FUNCTION '||FUNCNAME1; 
  execute immediate str ; 
end if; 
end;


CALL DELFUNC('NUMBER_TO_BIT');

CREATE FUNCTION NUMBER_TO_BIT
 ("V_NUM" INTEGER
 ) 
  RETURNS VARCHAR(16)
...
DROP PROCEDURE DELFUNC;

                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 
BEGIN ATOMIC 
DECLARE PIVERSION VARCHAR(32);
SET PIVERSION = '4.06.03';
IF NOT EXISTS (SELECT VERSION FROM VERSION_CMPP WHERE VERSION = PIVERSION) THEN
INSERT INTO VERSION_CMPP (VERSION,VERSIONDATE,UPGRADETIME)
VALUES (PIVERSION,CURRENT DATE,(TRIM(CHAR(YEAR(CURRENT TIMESTAMP)))||'-'|| (CASE MONTH(CURRENT TIMESTAMP) WHEN 
1 THEN '01' WHEN 2 THEN '02' WHEN 3 THEN '03' WHEN 4 THEN '04' WHEN 5 THEN '05' WHEN 6 THEN '06'WHEN 7 THEN '07' WHEN 8 THEN '08' WHEN 9 THEN '09'   ELSE   
...
END IF;
END ;     
/*V4.06.02-V4.06.03 END*/

/*V4.06.03-V4.06.04 START*/
...
END IF;
END ;     
/*V4.06.03-V4.06.04 END*/

/*V4.06.04-V4.06.05 START*/
       
MERGE INTO  A_GWPARAMCONF A USING (SELECT 'MTBUFSIZEN' AS "PARAMITEM",3000 AS "GWTYPE" FROM  SYSIBM.DUAL) B ON (A.PARAMITEM=B.PARAMITEM AND A.GWTYPE=B.GWTYPE)
WHEN NOT MATCHED THEN 
...
DECLARE NUMSTR INT;
DECLARE TOTALSTR INT;
SET VERSIONSTR = '6.7.0.371';
SET DBVERSIONSTR='6.39';
SET WBSVERSIONSTR='8.5.0.198';   
SET SPGATEVERSIONSTR='6.1.30.322';
SET NUMSTR=1;
SET TOTALSTR=1;
--EMP产品版本记录
...
VALUES(1000,1000,VERSIONSTR,CURRENT TIMESTAMP,CURRENT TIMESTAMP,'EMP-WEB',1,'1、网关性能优化。<br />');
ELSE
UPDATE LF_VERSION_HIS SET UPDATETIME=CURRENT TIMESTAMP,ISRELEASE=1,VERSIONINFO='1、网关性能优化。<br />' WHERE VERSION=VERSIONSTR AND PRODUCT_ID=1000 AND PROCESS_ID=1000;
END IF;
