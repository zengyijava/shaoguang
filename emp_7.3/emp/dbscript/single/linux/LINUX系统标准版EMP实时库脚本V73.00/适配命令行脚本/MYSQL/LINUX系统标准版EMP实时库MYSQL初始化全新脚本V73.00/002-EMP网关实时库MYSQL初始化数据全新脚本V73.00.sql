...
CREATE DEFINER=`root`@`%` PROCEDURE `CHECK_INSERT`()
BEGIN
IF NOT EXISTS (SELECT 1 FROM GW_USERPROPERTY WHERE USERID IN (SELECT USERID FROM USERDATA WHERE USERTYPE=0 AND NOT EXISTS (SELECT USERID FROM GW_USERPROPERTY  WHERE USERID=USERDATA.USERID)) AND ECID = 100001) THEN
...
END$
CALL CHECK_INSERT()$
DROP PROCEDURE CHECK_INSERT$


DROP PROCEDURE IF EXISTS CHECK_INSERT$
CREATE DEFINER=`root`@`%` PROCEDURE `CHECK_INSERT`()
BEGIN
IF NOT EXISTS (SELECT 1 FROM GW_GATECONNINFO 
						WHERE PTACCID IN (SELECT PTACCID FROM A_GWACCOUNT A
	WHERE NOT EXISTS (SELECT PTACCID,IP,PORT  FROM GW_GATECONNINFO WHERE A.SPIP=IP AND A.PTACCID=PTACCID AND PORT=A.SPPORT)) 
						AND IP IN (SELECT SPIP FROM A_GWACCOUNT A
	WHERE NOT EXISTS (SELECT PTACCID,IP,PORT  FROM GW_GATECONNINFO WHERE A.SPIP=IP AND A.PTACCID=PTACCID AND PORT=A.SPPORT)) 
						AND PORT IN (SELECT SPPORT FROM A_GWACCOUNT A
	WHERE NOT EXISTS (SELECT PTACCID,IP,PORT  FROM GW_GATECONNINFO WHERE A.SPIP=IP AND A.PTACCID=PTACCID AND PORT=A.SPPORT))
						AND KEEPCONN = 0 AND TESTMETHOD = 2) THEN
...
CREATE DEFINER=`root`@`%` PROCEDURE `CHECK_INSERT`()
BEGIN
IF NOT EXISTS (SELECT 1 FROM PROCESSINGSTATUS WHERE USEID = 1 AND CURRINDEX = 0 AND MAXINDEX = 0 AND COUNTSTATUS = 0 AND DISTRACTSTATUS = 0 AND DELETESTATUS = 0) THEN
	INSERT INTO PROCESSINGSTATUS (USEID,CURRINDEX,MAXINDEX,COUNTSTATUS,DISTRACTSTATUS,DELETESTATUS)VALUES(1,0,0,0,0,0);
END IF;
IF NOT EXISTS (SELECT 1 FROM PROCESSINGSTATUS WHERE USEID = 2 AND CURRINDEX = 0 AND MAXINDEX = 0 AND COUNTSTATUS = 0 AND DISTRACTSTATUS = 0 AND DELETESTATUS = 0) THEN
...
##创建三年历史表
DROP PROCEDURE IF EXISTS `CHECK_CREATE_HISTAB`$
CREATE DEFINER=`root`@`%` PROCEDURE `CHECK_CREATE_HISTAB`()
BEGIN
    DECLARE PITYPE INT;
...
WHILE PICURTYPE<=PITYPE DO
  ...
  WHILE PICURNUM<PINUM DO
	...
	IF NOT EXISTS (SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME=PITABLENAME) THEN
	 IF EXISTS( SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = PIYM ) THEN
	 IF PICURTYPE=0 THEN
...
END WHILE;
END$
CALL `CHECK_CREATE_HISTAB`()$
DROP PROCEDURE `CHECK_CREATE_HISTAB`$
