...
ALTER TABLE LF_MTTASK ADD VALIDTM  SMALLINT DEFAULT 48 NOT NULL;
...
/* EMP升级脚本START */
CREATE INDEX "IND_LF_RMS_REP_IYMD" ON "LF_RMS_REPORT"
(
	"IYMD" ASC
);CREATE INDEX "IND_LF_RMS_REP_DEG" ON "LF_RMS_REPORT"
(
	"DEGREE" ASC
);
CREATE INDEX "IND_LF_RMS_REP_SPCM" ON "LF_RMS_REPORT"
(
	"SPISUNCM" ASC
);CREATE INDEX "IND_LF_RMS_REP_UID" ON "LF_RMS_REPORT"
(
	"USERID" ASC
);
...
ALTER TABLE LF_CORP ADD RPTFLAG INTEGER NOT NULL WITH DEFAULT 1;
...
ALTER TABLE LF_INDUSTRY_USE ADD TMPTYPE DECIMAL(11) DEFAULT 0 NOT NULL;
...
ALTER TABLE LF_MTTASK ADD TEMP_TYPE DECIMAL(11) DEFAULT 0 NOT NULL;
...
CREATE TABLE LF_URLTASK(
  ...
  CREATE INDEX IX_LF_URLTASK_TASK ON LF_URLTASK(TASKID);
...
CREATE INDEX INDEX_TMID_TEMPCONTENT ON LF_TEMPCONTENT(TMID ASC) PCTFREE 10 ALLOW REVERSE SCANS;
...
THEN
INSERT INTO LF_VERSION_HIS(PRODUCT_ID,PROCESS_ID,VERSION,UPDATETIME,CREATETIME,MEMO,ISRELEASE,VERSIONINFO)
VALUES(1000,1000,VERSIONSTR,CURRENT TIMESTAMP,CURRENT TIMESTAMP,'EMP-WEB',1,'1、修改富信发送页面。<br />2、添加公共模板。<br />3、优化菜单显示<br />');
ELSE
UPDATE LF_VERSION_HIS SET UPDATETIME=CURRENT TIMESTAMP,ISRELEASE=1,VERSIONINFO='1、修改富信发送页面。<br />2、添加公共模板。<br />3、优化菜单显示<br />' WHERE VERSION=VERSIONSTR AND PRODUCT_ID=1000 AND PROCESS_ID=1000;
END IF;
--EMP_GATEWAY
IF NOT EXISTS(SELECT * FROM LF_VERSION_HIS WHERE VERSION=WBSVERSION AND PRODUCT_ID=1000 AND PROCESS_ID=2000)
THEN
