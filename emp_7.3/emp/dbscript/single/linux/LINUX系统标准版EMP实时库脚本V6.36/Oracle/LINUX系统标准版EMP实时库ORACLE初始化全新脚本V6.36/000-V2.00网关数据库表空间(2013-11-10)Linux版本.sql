...
begin
  file_size := '1024M';  --创建的数据文件大小，默认1024M
  os_type:=1;		--!!!!!! 这里可以修改操作系统类型 !!!!!!
  file_path:='';
  ...
  --调整临时表空间的大小至合适的值
  file_name := ''''|| file_path||'temp01.dbf'||'''';
  str := 'ALTER DATABASE '||
      ' TEMPFILE '||file_name||' RESIZE  '||
      file_size;
  dbms_output.put_line(str);
  execute immediate str;
  
  file_name := ''''|| file_path||'temp02.dbf'||'''';
  str := 'ALTER TABLESPACE "TEMP" '||
      ' ADD '||
      ' TEMPFILE '||file_name ||
      ' SIZE '|| file_size ||' AUTOEXTEND '||
      ' ON NEXT '|| file_size;
  ...
  --调整undotbs
  file_name := ''''|| file_path||'undotbs01.dbf'||'''';
  str := 'ALTER DATABASE '||
      ' DATAFILE '||file_name||' RESIZE  '||
      file_size;
  dbms_output.put_line(str);
  execute immediate str;
  file_name := ''''|| file_path||'undotbs02.dbf'||'''';
  str := 'ALTER TABLESPACE "UNDOTBS1" '||
      ' ADD '||
      ' DATAFILE '||file_name ||
      ' SIZE '|| file_size ||' AUTOEXTEND '||
      ' ON NEXT '|| file_size;
  dbms_output.put_line(str);
  execute immediate str;
  file_name := ''''|| file_path||'undotbs03.dbf'||'''';
  str := 'ALTER TABLESPACE "UNDOTBS1" '|| 
      ' ADD '||
      ' DATAFILE '||file_name ||
      ' SIZE '|| file_size ||' AUTOEXTEND '||
  ...
  dbms_output.put_line(str);
  execute immediate str;
  file_name := ''''|| file_path||'undotbs04.dbf'||'''';
  str := 'ALTER TABLESPACE "UNDOTBS1" '|| 
      ' ADD '||
      ' DATAFILE '||file_name ||
      ' SIZE '|| file_size ||' AUTOEXTEND '||
  ...
  dbms_output.put_line(str);
  execute immediate str;
  file_name := ''''|| file_path||'undotbs05.dbf'||'''';
  str := 'ALTER TABLESPACE "UNDOTBS1" '|| 
      ' ADD '||
      ' DATAFILE '||file_name ||
      ' SIZE '|| file_size ||' AUTOEXTEND '||
  ...
  dbms_output.put_line(str);
  execute immediate str;
  file_name := ''''|| file_path||'undotbs06.dbf'||'''';
  str := 'ALTER TABLESPACE "UNDOTBS1" '|| 
      ' ADD '||
      ' DATAFILE '||file_name ||
      ' SIZE '|| file_size ||' AUTOEXTEND '||
  ...
  --调整system表空间
  file_name := ''''|| file_path||'system01.dbf'||'''';
  str := 'ALTER DATABASE '||
      ' DATAFILE '||file_name||' RESIZE  '||
      file_size;
  dbms_output.put_line(str);
  execute immediate str;
  file_name := ''''|| file_path||'system02.dbf'||'''';
  str := 'ALTER TABLESPACE "SYSTEM" '||
      ' ADD '||
      ' DATAFILE '||file_name ||
      ' SIZE '|| file_size ||' AUTOEXTEND '||
      ' ON NEXT '|| file_size;
  ...
  --调整sysaux表空间
  file_name := ''''|| file_path||'sysaux01.dbf'||'''';
  str := 'ALTER DATABASE '||
      ' DATAFILE '||file_name||' RESIZE  '||
      file_size;
  file_name := ''''|| file_path||'sysaux02.dbf'||'''';
  dbms_output.put_line(str);
  execute immediate str;
  str := 'ALTER TABLESPACE "SYSAUX" '||
      ' ADD '||
      ' DATAFILE '||file_name ||
      ' SIZE '|| file_size ||' AUTOEXTEND '||
      ' ON NEXT '|| file_size;
