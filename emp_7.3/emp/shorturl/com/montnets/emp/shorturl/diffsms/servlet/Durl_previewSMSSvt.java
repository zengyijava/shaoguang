...
public class Durl_previewSMSSvt extends BaseServlet
{
...
//			String taskId = fieldInfo.get("taskId");
			...
			//TOWR 加入短地址逻辑
			boolean re = smsAtom.parsePhoneAndContent(readerList, preParams, haoduan, lguserid.toString(), lgcorpcode, busCode, msg, spUser,domainId,taskId);
			if (!re) {
				result = "noShortUrl";
				return;
			}
			...
			if(preParams.getEffCount() > 1000000l)
			{
				...
				// 文件内有效号码大于500万
				result = "overstep";
				EmpExecutionContext.error("不同内容预览，有效号码数超过" + 1000000 + "，无法进行发送，taskid:"+taskId+"，corpCode:"+lgcorpcode+"，userid："+strlguserid);
				return;
			}
			...
			// 获取预发送条数
			preSendCount = smsBiz.countAllOprSmsNumByDsm(spUser, msg, Integer.parseInt(sendType), phoneFilePath[6], null);
...
//			}
			...
			request.getRequestDispatcher(this.empRoot + "/durlsms/dsm_preview.jsp").forward(request, response);
		}
	}
	...
	public void toDraft(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try
		{
			...
			if(preParams.getEffCount() > 1000000l)
			{
				...
				// 文件内有效号码大于500万
				result = "overstep";
				EmpExecutionContext.error("不同内容暂存草稿，有效号码数超过" + 1000000 + "，taskid:"+taskId+"，corpCode:"+lgcorpcode+"，userid："+strlguserid);
				return;
			}
...
//			}
			...
			preParams = null;
			request.setAttribute("result", result);
			request.getRequestDispatcher(this.empRoot + "/durlsms/dsm_drafts.jsp").forward(request, response);
		}
	}
