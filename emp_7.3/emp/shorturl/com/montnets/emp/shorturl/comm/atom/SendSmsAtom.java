package com.montnets.emp.shorturl.comm.atom;
...
import com.montnets.emp.shorturl.surlmanage.util.ExcelTool;
import com.montnets.emp.shorturl.surlmanage.util.GenUrl;
...
public class SendSmsAtom {
    ...
    //存放短链接发送详情的缓存
    //private LoadingCache<String, LfUrlVisit> cache = UrlCache.getInstance().getCache();
    //获取短链接的队列
    //private LinkedBlockingQueue<String> queue = UrlCache.getInstance().getCode();
    ...
    public List<BufferedReader> parseFile(FileItem fileItem, String url, int fileCount, PreviewParams preParams) throws Exception {
        ...
        try {
            ...
            // 解析2003及WPS格式的的excel文件
                ...
                //调用2003文件解析方法
                BufferedReader bufferedReader = excelTool.jxExcel2003_V1(url, fileItem.getInputStream(), "_" + String.valueOf(fileCount), preParams);
                //解析成功
            ...
            // 解析excel2007文件
                ...
                //调用2007文件解析方法
                BufferedReader bufferedReader = excelTool.jxExcel2007_V1(url, fileItem.getInputStream(), "_" + String.valueOf(fileCount), preParams, -1);
                //解析成功
                if (bufferedReader != null) {
    ...
    public List<BufferedReader> parseRar(FileItem fileItem, String url, int fileCount, PreviewParams params) throws Exception {
        ...
        try {
            ...
            if (rarUrl != null) {
                try {
                    boolean ra=rarUrl.delete();
                    if(!ra){
                        EmpExecutionContext.error( "删除rar文件失败!rarUrl:"+ rarUrl);
                    }
                } catch (Exception e) {
...
//					archive.close();
//				}catch(IOException e){
//					EmpExecutionContext.error(e, "Archive关闭异常");
//				}
//			}
        }
    }
     ...
     * @throws Exception
     */
    public boolean parsePhone(List<BufferedReader> readerList, PreviewParams params, String[] haoduan, String lguserid, String corpCode, String busCode, String msg, String domainId, String taskId) throws Exception {
        // 运营商有效号码数
        ...
        //短链接详情文件
        File perUrlFile = null;
        //无效号码文件
        File perBadFile = null;
        //合法号码文件流
        FileOutputStream perEffOS = null;
        //短链接详情文件流
        FileOutputStream perUrlOS = null;
        //无效号码文件流
        ...
        try {
            ...
            // 有效短链接详情 caizg
            StringBuffer urlContentSb = new StringBuffer();
            StringBuffer viewContentSb = new StringBuffer();
            //获取短域名对象
            LfDomain domain = baseBiz.getById(LfDomain.class, domainId);
            for (int r = 0; r < readerList.size(); r++) {
                // 如果上传号码大于设置的最大值就不允许发送
                if (params.getEffCount() > StaticValue.MAX_PHONE_NUM) {
                    EmpExecutionContext.error("相同内容预览，有效号码数超过" + StaticValue.MAX_PHONE_NUM + "，corpCode:" + corpCode + "，userid：" + lguserid);
                ...
                while ((mobile = reader.readLine()) != null) {
                    ...
                    if ((index = phoneUtil.getPhoneType(mobile, haoduan)) < 0) {
                        badContentSb.append("格式非法：").append(mobile).append(line);
                        ...
                        // 1为重复号码
                        if (resultStatus == 1) {
                            badContentSb.append("重复号码：").append(mobile).append(line);
                        ...
                        // 检查是否是黑名单
                        badContentSb.append("黑名单号码：").append(mobile).append(line);
                        ...
                        perEffCount++;
                        perUrlCount++;
                        //获取短链接地址，超过5分钟没有返回结果，不在等待
                        //String shortUrl = queue.poll(5, TimeUnit.MINUTES);
                        //String shortUrl = UrlCache.getInstance().getCode().poll(5, TimeUnit.MINUTES);

//						if (StringUtils.isBlank(shortUrl)) {
//							return false;
//						}
//						//拼接短地址
//						String changeUrl = UrlGlobals.CHANGE_URL+shortUrl;
//						//替换短信内容中原始活动地址
                        String text = msg;
//						text = text.replace(srcUrl, changeUrl);
                        // 短信内容BASE64编码
                        smsContentBase64 = contentBase64Encoder(text);
                        // 短信内容BASE64编码异常
                        if (smsContentBase64 == null) {
                            // 动态模板提交时，如果动态内容拼接处理异常
                            badContentSb.append("格式非法(发送短信内容格式错误)：").append(text).append(line);
                            params.setBadModeCount(params.getBadModeCount() + 1);
                            params.setBadCount(params.getBadCount() + 1);
                            perBadCount++;
                            continue;
...
//						// 有效号码
                        //contentSb.append(mobile+","+text).append(line);
//						urlContentSb.append(mobile).append("|").append(taskId)
//								.append("|").append(shortUrl).append("|")
//								.append(srcUrl).append("|").append(line);
                        // 详情放入缓存
                        //cache.put(shortUrl, new LfUrlVisit(shortUrl, srcUrl, Long.valueOf(taskId), mobile,0));
                        //UrlCache.getInstance().getCache().put(shortUrl, new LfUrlDetail(shortUrl, srcUrl, Long.valueOf(taskId), mobile,0));
                        ...
                        if (params.getEffCount() < 11) {
                            ...
                            String textView = getChangeMark(domain, text);
                            viewContentSb.append(mobile + "MWHS]#" + textView.replace("\r\n", " ")).append(line);
                        }
                        // 累加运营商有效号码数(区分运营商)
                    ...
                    if (perEffCount >= StaticValue.PER_PHONE_NUM) {
                        ...
                        if (perEffFile == null) {
                            ...
                            perEffFile = new File(params.getPhoneFilePath()[5]);
                            perUrlFile = new File(params.getPhoneFilePath()[0]);
                            if (!perUrlFile.exists()) {
                                boolean flag = perUrlFile.createNewFile();
                                if (!flag) {
                        ...
                        if (perEffOS == null) {
                            ...
                            perEffOS = new FileOutputStream(params.getPhoneFilePath()[5], true);
                        }
                        //写入有效号码文件
...
//					if (perUrlCount >= StaticValue.PER_PHONE_NUM) {
//						if(perUrlFile == null)
//						{
//							//有效号码文件
//							perUrlFile = new File(params.getPhoneFilePath()[5]);
//							//判断文件是否存在，不存在就新建一个
//							if (!perUrlFile.exists())
//							{
//								perUrlFile.createNewFile();
//							}
//						}
//						if(perUrlOS == null)
//						{
//							//合法号码文件输出流
//							perUrlOS = new FileOutputStream(params.getPhoneFilePath()[5], true);
//						}
//						//写入有效号码文件
//						//txtFileUtil.writeToTxtFile(params.getPhoneFilePath()[0], contentSb.toString());
//						txtFileUtil.repeatWriteToTxtFile(perUrlOS, urlContentSb.toString());
//						urlContentSb.setLength(0);
//						perUrlCount = 0;
//					}
...
//			}
            ...
            if (perUrlOS != null) {
                try {
                    ...
                    EmpExecutionContext.error(e, "相同内容发送，解析文本文件流关闭短链接详情文件输入流异常，lguserid:" + lguserid + "，corpCode:" + corpCode);
                }
            }
            // 执行删除临时文件的操作
    ...
    private String getChangeMark(LfDomain domain, String text) {
        //获取需要替换的标识
        try {
            String mark = domain.getDomain() + getAsterist(domain.getLenExten());
            String changeMark = domain.getDomain() + getShortUrl(domain.getLenExten());
            String textView = text.replace(mark, changeMark);
            return textView;
        } catch (Exception e) {
            EmpExecutionContext.error(e, "获取替换标识异常");
        }
        return text;
    }

    private String getShortUrl(Long lenExten) {
        if (lenExten == null) {
            return "";
        }
        long rnum = (long) (Math.random() * 100);
        String shortUrl = GenUrl._10_to_62(rnum, lenExten.intValue());
        if (shortUrl.length() > lenExten) {
            shortUrl = shortUrl.substring(0, lenExten.intValue());
        }
        return "/" + shortUrl;
    }
     ...
     * @throws Exception
     */
    public void parsePhoneAndContent(List<BufferedReader> readerList, PreviewParams params, String[] haoduan, String lguserid, String corpCode, String busCode, String dtMsg, String domainId) throws Exception {
        // 解析号码文件
        String tmp;
        String smsContent = "";
        // Base64编码后的短信内容
            ...
            for (int r = 0; r < readerList.size(); r++) {
                ...
                while ((tmp = reader.readLine()) != null) {
                    ...
                    if (index != -1) {
                        ...
                        badContentSb.append("格式非法(短信内容长度为零)：").append(tmp).append(line);
                        params.setBadModeCount(params.getBadModeCount() + 1);
                        params.setBadCount(params.getBadCount() + 1);
                        perBadCount++;
                        continue;
                    ...
                    if (phoneUtil.getPhoneType(mobile, haoduan) < 0) {
                        badContentSb.append("格式非法(手机号码号段或号码格式错误)：").append(tmp).append(line);
                        params.setBadModeCount(params.getBadModeCount() + 1);
                        params.setBadCount(params.getBadCount() + 1);
                        perBadCount++;
                        ...
                        //-1为非法号码
                            ...
                            badContentSb.append("格式非法(手机号码格式不正确)：").append(tmp).append(line);
                            params.setBadModeCount(params.getBadModeCount() + 1);
                            params.setBadCount(params.getBadCount() + 1);
                            perBadCount++;
                        }
                        ...
                        // 动态模板发送时，需验证文件参数个数是否匹配模板参数个数
                            ...
                            if (paramCount < params.getTempParamCount()) {
                                ...
                                badContentSb.append("格式非法(发送文件参数少于模板参数个数)：").append(tmp).append(line);
                                params.setBadModeCount(params.getBadModeCount() + 1);
                                params.setBadCount(params.getBadCount() + 1);
                                perBadCount++;
                                continue;
                        ...
                        if (smsContent.length() > StaticValue.MAX_MSG_LEN || smsContent.length() == 0) {
                            badContentSb.append("格式非法（当前长度：" + smsContent.length() + "）(允许长度为1-" + StaticValue.MAX_MSG_LEN + "个字符)：").append(tmp).append(line);
                            params.setBadModeCount(params.getBadModeCount() + 1);
                            params.setBadCount(params.getBadCount() + 1);
                            perBadCount++;
                            // 跳转下一循环
                        ...
                        // 过滤内容关键字
                        if (filterKeyRes == 1) {
                            badContentSb.append("包含关键字：").append(tmp).append(line);
...
//						}
                        ...
                        if (smsContentBase64 == null) {
                            // 动态模板提交时，如果动态内容拼接处理异常
                            badContentSb.append("格式非法(发送短信内容格式错误)：").append(tmp).append(line);
                            params.setBadModeCount(params.getBadModeCount() + 1);
                            params.setBadCount(params.getBadCount() + 1);
                            perBadCount++;
                            continue;
                        ...
                        // 预览10个号码
                        if (params.getEffCount() < 11) {
                            String textView = getChangeMark(domain, smsContent);
                            if (sendType == 2) {
                                // 预览信息
...
//			}
     ...
     * @throws Exception
     */
    public boolean parsePhoneAndContent(List<BufferedReader> readerList, PreviewParams params, String[] haoduan, String lguserid, String corpCode, String busCode, String dtMsg, String spUser, String domainId, String taskId) throws Exception {
        // 解析号码文件
        String tmp;
        // 短信内容
        String smsContent = "";
...
//								smsContent = combineContentSg(smsContent, dtMsg);
                                ...
                                if (smsContent == null) {
                                    // 动态模板提交时，如果动态内容拼接处理异常
                                    badContentSb.append("格式非法(发送短信内容与文件参数不符)：").append(tmp).append(line);
                                    params.setBadModeCount(params.getBadModeCount() + 1);
                                    params.setBadCount(params.getBadCount() + 1);
                                    perBadCount++;
                                    continue;
                        ...
                        if (phoneType == 3 && spGate != null) {
                            ...
                            if (("0".equals(SmsCharType) && smsLen > (720 - 20)) || ("1".equals(SmsCharType) && smsContent.length() > (360 - 10))) {
                                badContentSb.append("格式非法（当前长度：" + smsContent.length() + "，已超过国外通道短信最大长度)：").append(tmp).append(line);
                                params.setBadModeCount(params.getBadModeCount() + 1);
                                params.setBadCount(params.getBadCount() + 1);
                                perBadCount++;
                                // 跳转下一循环
...
//						String changeUrl = UrlGlobals.CHANGE_URL+shortUrl;
//						//替换短信内容中原始活动地址
//						//msg = msg.replace(srcUrl, changeUrl);
//						smsContent = smsContent.replace(srcUrl, changeUrl);
//						urlContentSb.append(mobile).append("|").append(taskId)
//								.append("|").append(shortUrl).append("|")
//								.append(srcUrl).append("|").append(line);
                        // 详情放入缓存
...
//						}
                        ...
                        if (params.getEffCount() < 11) {
                            ...
                            if (sendType == 2) {
                                ...
                                viewContentSb.append(mobile + "MWHS]#" + textView).append(line);
                            }
                        }
                    }
...
//			}
            ...
            if (perUrlOS != null) {
                try {
                    ...
                    EmpExecutionContext.error(e, "不同内容发送，解析文本文件流关闭短地址详情文件输入流异常，lguserid:" + lguserid + "，corpCode:" + corpCode);
                }
            }
            // 执行删除临时文件的操作
            cleanTempFile(params);
            ...
            if (delFileList != null && delFileList.size() > 0) {
                for (String fileUrl : delFileList) {
                    ...
                    if (file.exists()) {
                        ...
                        if(!re){
                            EmpExecutionContext.error( "删除临时文件失败");
                        }
                    }
                }
            }
     ...
     * @throws Exception
     */
    public boolean parseAddrAndInputPhone(PreviewParams params, String depIdStr, String phoneStr, String groupStr, String lgcorpcode, String[] haoduan, String busCode, String msg, String domainId, String taskId) throws Exception {
        try {
            // 运营商有效号码数
            int[] oprValidPhone = params.getOprValidPhone();
            // 运营商标识。0:移动号码 ;1:联通号码;2:电信号码;3:国际号码
            ...
            for (String num : phones) {
                ...
                if (num != null) {
                    if (num.length() >= 7 && num.length() <= 21) {
                        ...
                        //获取短链接地址，超过5分钟没有返回结果，不在等待
//						String shortUrl = queue.poll(5, TimeUnit.MINUTES);
////						LinkedBlockingQueue<String> queue = UrlCache.getInstance().getCode();
////						String shortUrl = queue.poll(5, TimeUnit.MINUTES);
//						
//						if (StringUtils.isBlank(shortUrl)) {
//							return false;
...
//						text = text.replace(srcUrl, changeUrl);
                        ...
                        // 有效号码
                        contentSb.append(num + "," + smsContentBase64).append(line);
//						// 有效号码
                        //	contentSb.append(num+","+text).append(line);
//						urlContentSb.append(num).append("|").append(taskId)
//								.append("|").append(shortUrl).append("|")
//								.append(srcUrl).append("|").append(line);
                        // 详情放入缓存
                        //cache.put(shortUrl, new LfUrlVisit(shortUrl, srcUrl, Long.valueOf(taskId), num,0));
                        ...
                        if (params.getEffCount() < 11) {
                            ...
                            phoneString = phoneString + num + ";";
                            String textView = getChangeMark(domain, text);
                            viewContentSb.append(num + "MWHS]#" + textView.replace("\r\n", " ")).append(line);
                        }
                    } else {
                        badContentSb.append("格式非法：").append(num).append(line);
                        params.setBadModeCount(params.getBadModeCount() + 1);
            ...
            if (contentSb.length() > 0) {
                ...
                txtFileUtil.writeToTxtFile(params.getPhoneFilePath()[5], contentSb.toString());
                contentSb.setLength(0);
            }
...
//			if(urlContentSb.length() > 0){
//				txtFileUtil.writeToTxtFile(params.getPhoneFilePath()[5], urlContentSb.toString());
//				urlContentSb.setLength(0);
//			}
            if (viewContentSb.length() > 0) {
                // 将预览信息写入文件
