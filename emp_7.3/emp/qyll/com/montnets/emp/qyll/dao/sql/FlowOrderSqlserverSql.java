...
public class FlowOrderSqlserverSql implements IFlowOrderSql{
	...
	public String getFindOrderDetailSql(int row) {
		String year = sdf.format(new Date());
		return  "select top "+row+" a.id,a.mobile,a.orderno,a.productid, " +
				"   a.msg, b.sp_user , b.sp_pwd  " +
				"   from ll_order_detail"+ year +
				"	a left join  ll_order_task b on a.orderno = b.orderno " +
				"	where a.Status = 0 and a.orderno = ? and b.servernum = ? " ;
	}
	...
	public String findOrderNumUnsentSql() {
		return "SELECT TOP 500 ORDERNO,USER_ID,TIMER_TIME,RE_STATUS,TIMER_STATUS,ISRETRY FROM LL_ORDER_TASK WHERE TIMER_TIME < ? AND RE_STATUS IN ('0','2') AND SERVERNUM = '0' AND ORDERSTATUS NOT IN ('3','4','5') " ;
	}
