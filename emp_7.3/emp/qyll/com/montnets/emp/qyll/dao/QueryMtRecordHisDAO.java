 ...
 * @description
 */
public class QueryMtRecordHisDAO extends SuperDAO
{
	...
	public List<DynaBean> findMtTasksHis(LinkedHashMap<String, String> conditionMap, PageInfo pageInfo)
	{
		try
		{
			...
			//组装过滤条件语句
			String conditionSql = QueryMtRecordDAOSql.getConditionSql(conditionMap, "mttaskxx");
			//时间段的过滤条件
			List<String> timeList = QueryMtRecordDAOSql.getTimeCondition(conditionMap);
			//组sql语句
	...
	public List<DynaBean> findMtTasksHis(LinkedHashMap<String, String> conditionMap, PageInfo pageInfo, PageInfo realDbpageInfo)
	{
		...
		{
			//获取查询的列名
			String fieldSql = "select mttaskxx.ID,mttaskxx.USERID,mttaskxx.SPGATE,mttaskxx.CPNO,mttaskxx.PHONE,mttaskxx.SENDSTATUS,mttaskxx.TASKID,mttaskxx.ERRORCODE,mttaskxx.SENDTIME,mttaskxx.MESSAGE,mttaskxx.UNICOM,mttaskxx.PKNUMBER,mttaskxx.PKTOTAL,mttaskxx.SVRTYPE,mttaskxx.RECVTIME,mttaskxx.P1,mttaskxx.P2,mttaskxx.usermsgid,mttaskxx.custid ";
			...
			EmpExecutionContext.info("查询下行记录，date:"+date
		    ...
		    if((StaticValue.getUseHistoryDBTime().equals(date) && "1".equals(use_his_server)))
		    {
				...
				else
				{
	    			...
	    			//返回历史库查询结果
	    			returnList = mtTaskDao.findPageDynaBeanBySQL(sql, null, pageInfoTemp, StaticValue.EMP_BACKUP, timeList);
	    			return returnList;
				}
		    }
		    else
		    {
		    	...
		    	returnList = mtTaskDao.findPageDynaBeanBySQL(sql, null, pageInfo, poolName, timeList);
		    	return returnList;
		    }
		}
		catch (Exception e)
	...
	public boolean findMtTasksHisPageInfo(LinkedHashMap<String, String> conditionMap, PageInfo pageInfo, PageInfo realDbpageInfo)
	{
		try
		{
			...
			if((StaticValue.getUseHistoryDBTime().equals(date) && "1".equals(use_his_server)))
			{
	    		mtTaskDao.findPageInfoBySQL(countSql, pageInfo, StaticValue.EMP_POOLNAME, timeList);
    			//设置实时库的总页数
		    	...
		    	//设置查询一页条数
		    	pageInfoTemp.setPageSize(pageInfo.getPageSize());
		    	mtTaskDao.findPageInfoBySQL(countSql, pageInfoTemp, StaticValue.EMP_BACKUP, timeList);
			...
			else
		    {
		    	...
		    	return mtTaskDao.findPageInfoBySQL(countSql, pageInfo, poolName, timeList);
		    }
	...
	private String getMttaskHisTableSql(LinkedHashMap<String, String> conditionMap) 
	{
		try
		{
			...
			//两个表都存在，则用联合查询
			//查询列
			String fieldSql = "select ID,userid,spgate,cpno,phone,sendstatus,taskid,errorcode,sendtime,message,unicom,pknumber,pktotal,svrtype,recvtime,p1,p2,usermsgid ";
			String unionSql = new StringBuffer()
					.append("(")
					.append(fieldSql).append(" from ").append(tableName).append(StaticValue.getWITHNOLOCK())
					.append(" union all ")
					.append(fieldSql).append(" from ").append(secondTableName).append(StaticValue.getWITHNOLOCK())
					.append(")").toString();
