...
import com.montnets.emp.weix.dao.i.IAccountDao;
...
public class AccountDao extends SuperDAO implements IAccountDao
{
	...
	public List<DynaBean> findAllAccountByCorpCode(String corpCode, LinkedHashMap<String, String> conditionMap, LinkedHashMap<String, String> orderbyMap, PageInfo pageInfo) throws Exception
	{
		...
		tableSql.append(" FROM ").append(TableLfWcAccount.TABLE_NAME).append(" lfAccount ").append(" WHERE");
		// 查询条件
		...
		if(conditionMap != null && !conditionMap.entrySet().isEmpty())
		{
			...
			{
				conSql.append(" and lfAccount." + TableLfWcAccount.NAME + " like '%" + conditionMap.get("name") + "%' ");
			}
			if(conditionMap.get("ghname") != null && !"".equals(conditionMap.get("ghname")))
			{
				conSql.append(" and lfAccount." + TableLfWcAccount.OPEN_ID + " like '%" + conditionMap.get("ghname") + "%' ");
			}
		}
		// 排序
