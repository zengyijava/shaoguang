...
import com.montnets.emp.weix.dao.i.IMsgDao;
...
public class MsgDao extends SuperDAO implements IMsgDao
{
	/**
	...
	public List<DynaBean> findListMsgByCondition(String corpCode, LinkedHashMap<String, String> conditionMap, LinkedHashMap<String, String> orderbyMap, PageInfo pageInfo) throws Exception
	{
		...
		// 拼SQL语句
		String fieldSql = "SELECT lfMsg.*,lfAccount.NAME as acctName,lfuser.NAME as userName,lfuser.CODE as userCode,alink.OPEN_ID as openid";
		StringBuffer tableSql = new StringBuffer();
		tableSql.append(" FROM ").append(tableName).append(" lfMsg ");

		tableSql.append(" LEFT JOIN ").append(TableLfWcAccount.TABLE_NAME).append(" lfAccount ").append(" ON ");
		tableSql.append("lfMsg.A_ID").append("=").append("lfAccount." + TableLfWcAccount.A_ID);
		tableSql.append(" LEFT JOIN ").append(TableLfWcUserInfo.TABLE_NAME).append(" lfuser ").append("ON ");
		tableSql.append("lfMsg.WC_ID").append("=").append("lfuser." + TableLfWcUserInfo.WC_ID);
		tableSql.append(" LEFT JOIN ").append(TableLfWcAlink.TABLE_NAME).append(" alink ").append("ON ");
		tableSql.append("(").append("lfMsg.WC_ID").append("=").append("alink." + TableLfWcAlink.WC_ID).append(")").append(" and ");
		tableSql.append("(").append("lfMsg.A_ID").append("=").append("alink." + TableLfWcAlink.A_ID).append(")");
		...
		if(conditionMap != null && !conditionMap.entrySet().isEmpty())
		{
			...
			{
				conSql.append(" and ( lfuser." + TableLfWcUserInfo.NAME + " like '%" + conditionMap.get("wcname") + "%'");
			}
			if(conditionMap.get("openid") != null && !"".equals(conditionMap.get("openid")))
			{
				conSql.append(" OR alink." + TableLfWcAlink.OPEN_ID + " like '%" + conditionMap.get("openid") + "%'").append(")");
			}
	...
	public List<DynaBean> findListMsgDefault(String corpCode, LinkedHashMap<String, String> conditionMap, Object orderbyMap, PageInfo pageInfo)
	{
		...
		StringBuffer tableSql = new StringBuffer();
		tableSql.append(" FROM ").append(TableLfWcMsg.TABLE_NAME).append(" lfMsg ");
		...
		StringBuffer conSql = new StringBuffer();
		conSql.append(" lfAccount.").append(TableLfWcAccount.CORP_CODE).append("='").append(corpCode).append("'");
		// 排序
		String orderbySql = " order by lfMsg.CREATETIME DESC ";
		// 返回结果
		String sql = fieldSql + tableSql.toString() + conSql.toString() + orderbySql;
	...
	public void createMsg(LfWcMsg msg) throws SQLException
	{
		...
		insertSql.append("(").append(TableLfWcMsg.MSG_ID).append(",");
		insertSql.append(TableLfWcMsg.MSG_TYPE).append(",");
		insertSql.append(TableLfWcMsg.WC_ID).append(",");
		insertSql.append(TableLfWcMsg.A_ID).append(",");
		insertSql.append(TableLfWcMsg.TYPE).append(",");
		insertSql.append(TableLfWcMsg.MSG_TEXT).append(",");
		insertSql.append(TableLfWcMsg.MSG_XML).append(',');
		insertSql.append(TableLfWcMsg.PARENT_ID).append(',');
		insertSql.append(TableLfWcMsg.CREATETIME).append(',');
		insertSql.append(TableLfWcMsg.CORP_CODE);
		insertSql.append(')');
